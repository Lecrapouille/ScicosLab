*DECK DBESJ1
      DOUBLE PRECISION FUNCTION DBESJ1SCI (X)
...
C***END PROLOGUE  DBESJ1
     ...
     1  D1MACH, DCSEVL
      ...
      DATA FIRST /.TRUE./
C***FIRST EXECUTABLE STATEMENT  DBESJ1SCI
      IF (FIRST) THEN
...
C
     ...
     +   'ABS(X) SO SMALL J1 UNDERFLOWS', 1, 1)
      IF (Y.GT.XMIN) DBESJ1SCI = 0.5D0*X
      IF (Y.GT.XSML) DBESJ1SCI = X*(.25D0 + DCSEVL (.125D0*Y*Y-1.D0,
     1  BJ1CS, NTJ1) )
      RETURN
C
 20   CALL D9B1MP (Y, AMPL, THETA)
      DBESJ1SCI = SIGN (AMPL, X) * COS(THETA)
C
      RETURN
      END
