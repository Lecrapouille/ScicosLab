*DECK DGAMMA
      DOUBLE PRECISION FUNCTION DGAMMASCI (X)
...
C
      ...
      Y = X - N
      N = N - 1
      DGAMMASCI = 0.9375D0 + DCSEVL (2.D0*Y-1.D0, GAMCS, NGAM)
      IF (N.EQ.0) RETURN
C
      IF (N.GT.0) GO TO 30
C
...
C
     ...
     +   'X SO SMALL GAMMA UNDERFLOWS', 2, 1)
      IF (X.LT.XMIN) RETURN
C
      DGAMMASCI = EXP ((Y-0.5D0)*LOG(Y) - Y + SQ2PIL + D9LGMC(Y) )
      IF (X.GT.0.D0) RETURN
C
...
C
      DGAMMASCI = -PI/(Y*SINPIY*DGAMMASCI)
C
      RETURN
      END
