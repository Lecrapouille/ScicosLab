...
C
C***REFERENCES  (NONE)
C***ROUTINES CALLED  D1MACH, D9LGMC, DGAMMASCI, XERMSG
...
C***END PROLOGUE  DLNGAM
     ...
     1  Y, DGAMMASCI, D9LGMC, D1MACH, TEMP
      LOGICAL FIRST
      EXTERNAL DGAMMASCI
...
C
C LOG (ABS (DGAMMASCI(X)) ) FOR ABS(X) .LE. 10.0
C
      DLNGAM = LOG (ABS (DGAMMASCI(X)) )
      RETURN
C
C LOG ( ABS (DGAMMASCI(X)) ) FOR ABS(X) .GT. 10.0
C
