*DECK DBESY1
      DOUBLE PRECISION FUNCTION  DBESY1SCI (X)
C***BEGIN PROLOGUE   DBESY1SCI
...
C***END PROLOGUE  DBESY1
     ...
     1  Y, D1MACH, DCSEVL, DBESJ1
      ...
      DATA FIRST /.TRUE./
C***FIRST EXECUTABLE STATEMENT   DBESY1SCI
      IF (FIRST) THEN
...
C
     ...
     +   'X SO SMALL Y1 OVERFLOWS', 3, 2)
      Y = 0.D0
      IF (X.GT.XSML) Y = X*X
       DBESY1SCI = TWODPI * LOG(0.5D0*X)*DBESJ1(X) + (0.5D0 +
     1  DCSEVL (.125D0*Y-1.D0, BY1CS, NTY1))/X
      RETURN
C
 20   CALL D9B1MP (X, AMPL, THETA)
       DBESY1SCI = AMPL * SIN(THETA)
      RETURN
C
      END
