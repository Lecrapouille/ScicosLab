  ...
  if ( first == 0 ) 
    {
      first++;
#ifdef WIN32 
	  if (!GetTempPath(L_PATH_MAX,TmpDirDefault))
	  {
	  ...
	  {
		  if (!GetShortPathName(TmpDirDefault,TmpDirDefault,L_PATH_MAX))
	  	  {
		  	  MessageBox(NULL,"Can''t convert Windows temporary directory to short name format","Error",MB_ICONERROR);
		  	  exit(1);
	  	  }
	  ...
	  if ( CreateDirectory(tmp_dir,NULL)==FALSE)
      {
		...
		else
		{
		  ...
		  GetTempPath(L_PATH_MAX,TmpDirDefault);
