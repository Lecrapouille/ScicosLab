...
/* these functions can be used on 64bit machines
 * since it returns adresses which can be stored in int
 * and this is necessary for scilab stack.
 */
...
void *malloc_rep(register size_t size)
{
...
void *realloc_rep(void *f, size_t size)
{
	mem *p;
	void *q, *f1;
	size_t s1;

	if (!f) return malloc_rep(size);
...
void *calloc_rep(size_t num, register size_t size)
{
  register void *p;
  size *= num;
  if ( (p = malloc_rep(size)) )
	bzero(p, size);
  return(p);
}
