...
#define link_round(x,s) (((x) + ((s)-1)) & ~((s)-1))
#ifndef hppa
#define RNDVAL1 4
#define RNDVAL2 1024
#else
...
#define TEXTBEGIN sizeof filehdr + filehdr.f_opthdr + \
   ...
   readsize = link_round(aouthdr.tsize, RNDVAL1) + link_round(aouthdr.dsize, RNDVAL1);
   ...
   readsize = link_round(header.a_text, RNDVAL1) + link_round(header.a_data, RNDVAL1);
   totalsize = readsize + header.a_bss;
#endif /* COFF */
   totalsize = link_round(totalsize, RNDVAL2);   /* round up a bit */
