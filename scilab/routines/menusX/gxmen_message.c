...
int ExposeMessageWindow(void)
{
  ...
  gtk_widget_set_name (window, SCILAB_NAME" message");
  ...
  gtk_window_set_title (GTK_WINDOW (window), SCILAB_NAME" message");
...
int ExposeMessageWindow1(void)
{
  int alloc; 
  GtkWidget *dialog;
  gchar *msg_utf8 = sci_convert_to_utf8(ScilabMessage.string,&alloc);
...
int ExposeMessageWindow(void)
{
  ...
  switch ( ScilabMessage.nb ) 
    ...
    case 1 : 
      dialog = gtk_dialog_new_with_buttons (SCILAB_NAME" Dialog", GTK_WINDOW (window),
					    GTK_DIALOG_MODAL| GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    default: 
      ...
      dialog = gtk_dialog_new_with_buttons (SCILAB_NAME" Dialog",
					    GTK_WINDOW (window),
					    GTK_DIALOG_MODAL| GTK_DIALOG_DESTROY_WITH_PARENT,
					    ok_mess, GTK_RESPONSE_OK,
