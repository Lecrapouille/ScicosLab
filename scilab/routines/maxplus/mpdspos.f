      subroutine mpdspos(op,ma,na,a,nela,inda,mb,nb,b,
c     Copyright INRIA
     $     nelc,indc,ierr)
c!purpose
...
c     !
      double precision a(*),b(mb,nb),ZERO
...
C$$$      subroutine spcmps(op, A_m, A_n, A_nel, A_R, A_mnel, A_icol,
C$$$     $                  s, C_nelmax, C_mnel, C_icol, ierr)
      
C$$$*     comparizon  A op scalaire (where "0 op s" is false)
C$$$*     added by bruno to speed up this operation
C$$$      implicit none
C$$$      integer op, A_m, A_n, A_nel, A_mnel(*), A_icol(*),
C$$$     $            C_nelmax, C_mnel(*), C_icol(*), ierr
C$$$      double precision A_R(*), s

C$$$      integer kA, kAf, kC, i, jA, k
...
C$$$         do k = kA, kAf
C$$$             call cmp_and_update(A_R(k), s, op, C_mnel(i),
C$$$     $                           C_icol, A_icol(k), kC, C_nelmax, ierr)
