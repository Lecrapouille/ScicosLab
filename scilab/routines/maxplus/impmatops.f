...
c     
      ...
      integer top0
      integer typecode, algtype2, algcode
...
c         if(it2+2*it1.eq.1) call unsfdcopy(mn1,stk(l2+mn2),0,
c     &                                 stk(l1+mn1),1)
...
c      if(it2+2*it1.eq.1) call unsfdcopy(mn1,stk(l2+mn1),1,stk(l1+mn1),1)
c      if(it1*it2.eq.1) call mdadd(ia1,
...
c
     ...
     &              mn2,csti,0,stk(l1+mn2),1)
      ...
      if(it2+2*it1.eq.1) call unsfdcopy(mn1,stk(l2+mn1),1,stk(l1+mn1),1)
      if(it1*it2.eq.1) call mdadd(2,
...
c      if(it1.eq.0) call unsfdcopy(mn1,stk(l2+mn1),1,stk(l1+mn1),1)
...
c
      ...
      istk(il2+2)=m2
c
      call unsfdcopy(vol,stk(l2),1,stk(ll),1)
...
c
      ...
      istk(il1+2)=m1
c
      call unsfdcopy(vol1,stk(l1),1,stk(ll),1)
...
c
 56   if(it1*it2.eq.1) goto 58
      ...
      if(it2.eq.1) call unsfdcopy(mn1,stk(l2+mn2),i2,stk(l1+mn1),i1)
      goto 999
c
c     a et a2 complexes
c
...
c     
     ...
     & stk(l1+mn1+mn2),-1)
         ...
         if(it1.eq.0) call unsfdcopy(mn2,stk(l2+mn2),1,
     & stk(l1+2*mn1+mn2),1)
         if(it2.eq.0) call dset(mn2,PCZERO,stk(l1+2*mn1+mn2),1)
         if(it2.eq.1) call unsfdcopy(mn2,stk(l2+mn2),1,
     & stk(l1+2*mn1+mn2),1)
      endif
      n=n1+n2
      istk(il1+1)=m1
...
c===================
c
c      GOTO 777 POUR TESTER NOUVEAU CODE
      ...
      if(n1.ne.n2.or.m1.ne.m2) then
         if(op.eq.11.or.op.eq.12.or.(op.le.47.and.op.ge.44)) then
            istk(il1)=4
            istk(il1+1)=1
            istk(il1+2)=1
            istk(il1+3)=0
            if(op.eq.12.or.op.eq.46.or.op.eq.47) istk(il1+3)=1
            lstk(top+1)=sadr(il1+4)
         else
            call error(60)
            return
...
c============================================
c     Comparaisons Nouveau Code (Pas Utilise
c     pour l(instant)
c============================================
 777  CONTINUE
c      write(*,*)'comp'
...
c     .  eye op b
         ...
         elseif(mn2.gt.0) then
            call dset(mn2,PCZERO,stk(lw),1)
            call dset(min(m2,n2),stk(l1),stk(lw),m2+1)
            if(it1.eq.1) then
               call dset(mn2,PCZERO,stk(lw+mn2),1)
      ...
      elseif(m2.eq.-1) then
         ...
         call dset(mn1,PCZERO,stk(lw),1)
         call dset(min(m1,n1),stk(l2),stk(lw),m1+1)
         if(it1.eq.1) then
            call dset(mn1,PCZERO,stk(lw+mn1),1)
...
c      write(*,*)'comp 4',mn2,mn1,op
      if(mn2.eq.0.or.mn1.eq.0) then
c         if(op.eq.equal.or.op.eq.less+great) then
         if(op.eq.44.or.op.eq.45.or.op.eq.46.or.op.eq.47) then
c            write(*,*)'COMP 3'
            itrue=0
            if(mn2.eq.0.and.mn1.eq.0) itrue=1
            if(op.eq.46.or.op.eq.47) itrue=1-itrue
            istk(il1)=4
            istk(il1+1)=1
            istk(il1+2)=1
            istk(il1+3)=itrue
