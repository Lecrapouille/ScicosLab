...
/* static void print_matrix(double *B,int n,int q); */
/* static void  print_vector(double *b,int n); */
/* static double norm(double *U, int n, int p); */
/* static int solve(double *A, int n, int p, double *B, double *C, int r, double **U,double precision);*/
static int rowbasis2(double *A,int n,double *B,double **U, double precision);
static int rowbasis3(double *A,int n,double *B,int p,double *U, double **UU, double precision);
static double scalar_star(double a);
static double secure_max_add(double a,double b,double c,double d);
static void product_line(int i, double *A, int n, int p, double *B, int q, double *C);
...
void weakbasis(double *A,int n,int p, double **B, int *q, double precision)
{
  ...
  /* begin first pass */
  /* printf("entering weakbasis with %d rows and %d generators\n",n,p); */
  for (i=0;i<n; i++)
    {
      C[i]=A[i];
    }
	...
	if ((B[i]<=A[i]+precision) && (A[j]<=B[j]+precision) )
	  { 
		...
		for (k=0; k<p; k++)
	      ...
	      {
		for (k=0; k<p; k++)
                  if ((B[j]==EPSILON)&& (i== j))/*  SG 2007 temporary debug */
		    {
		      /* printf("degenerate case\n"); */
		      V[R+k]=U[k+i*p]; /*  SG temporary debug */
		    } else
		    {
		      V[R+k]=U[k+i*p]+B[j];
		    }
