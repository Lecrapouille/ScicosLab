...
c----------------------------------------------------------
   20 continue
      ...
      istk(il+3)=0
      if(it.eq.1) call unsfdcopy(mn,stk(l+mn),1,stk(l),1)
...
c     diag(a,k)
      ...
      if(k.ge.0) call unsfdcopy(mn,stk(l+k*m),m+1,stk(l),1)
      if(k.lt.0) call unsfdcopy(mn,stk(l-k),m+1,stk(l),1)
      if(it.eq.0) goto 999
      if(k.ge.0) call unsfdcopy(mn,stk(l+m*n+k*m),m+1,stk(l+mn),1)
      if(k.lt.0) call unsfdcopy(mn,stk(l+m*n-k),m+1,stk(l+mn),1)
      go to 999
c     diag(vecteur,k)
   63 nn = max(m,n)+abs(k)
      l1=l+nn*nn*(it+1)
      ...
      if(k.ge.0) call unsfdcopy(mn,stk(l1),1,stk(l+nn*k),nn+1)
      if(k.lt.0) call unsfdcopy(mn,stk(l1),1,stk(l-k),nn+1)
      if(it.eq.0) goto 999
      if(k.ge.0) call unsfdcopy(mn,stk(l1+mn),1,stk(l+nn*nn+k*nn),nn+1)
      if(k.lt.0) call unsfdcopy(mn,stk(l1+mn),1,stk(l+nn*nn-k),nn+1)
      go to 999
c
c     tril, triu
...
c     maxi mini des vecteurs d'une liste
 ...
 102    continue
        stk(istk(ilw+1)+i)=real(ji)
 103  continue
      call unsfdcopy(n1+n2 ,stk(ld),1,stk(lstk(top)),1)
...
c move a and b
      ...
      call unsfdcopy(mnb*(itb+1),stk(lb),-1,stk(l1+mna*(ita+1)),-1)
      lb=l1+mna*(ita+1)
      call unsfdcopy(mna*(ita+1),stk(la),-1,stk(l1),-1)
      la=l1
 ...
 170  continue
      ...
      if(m.eq.n.and.n.gt.1) then
         ...
         if(it.eq.0) then
            ...
            call unsfdcopy(nn,stk(le),1,stk(l),1)
         else
            le=lstk(top+1)
            lw=le+2*nn
     ...
     *           n,stk(lw),istk(ilb),err)
            ...
            call unsfdcopy(2*nn,stk(le),1,stk(l),1)
         endif
         goto 999
      endif
      if(it.eq.0) then
...
c      write(*,*)"tropical 2"
         ...
         istk(il1)=algtype2(int(stk(l)),typecode(istk(il1)))
      endif
c     write(*,*)"istk(il1)"
c     write(*,*)istk(il1)
      goto 999
