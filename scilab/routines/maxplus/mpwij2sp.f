      subroutine mpwij2sp(m,n,nel,ij,vr,vi,ind,nind,iw,ierr)
      include 'mpstack.h'
c     Copyright INRIA
...
c     eliminate leading zero entries
      k0=0
 05   k0=k0+1
      if (vr(k0).eq.PCZERO.and.vi(k0).eq.PCZERO.and.k0.lt.nel) goto 05
      if (vr(k0).eq.PCZERO.and.vi(k0).eq.PCZERO) then
         nel1=0
         goto 10
      endif
      ...
      if(nel.gt.k0) then
      do 08 k=k0+1,nel
         if (vr(k0).ne.PCZERO.or.vi(k0).ne.PCZERO) then
