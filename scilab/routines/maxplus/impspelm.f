...
c      integer PCALGCODE
      ...
      integer id(nsiz)
      integer iadr, sadr,typecode,algtype2,fd
      logical getscalar
...
c     get logical unit
 ...
 50   if(.not.getscalar('load',top,top,lr)) return
      ...
      integer iadr, sadr,typecode,algtype2
c
      iadr(l)=l+l-1
      sadr(l)=(l/2)+1
c     
...
c     sparse(x)
         il=iadr(lstk(top))
         if(istk(il).eq.261.or.istk(il).eq.6) return
         m=istk(il+1)
         n=istk(il+2)
         if(m*n.eq.0.or.m.eq.-1) return
         if(istk(il).eq.257) then
            it=istk(il+3)
            l=sadr(il+4)
c     
            ilr=iadr(lw)
...
c            istk(il+2)=n
         ...
         elseif(istk(il).eq.4) then
            ...
            call funnam(ids(1,pt+1),'%talg_sparse',iadr(lstk(top)))
            fun=-1
            return
         endif
         return
...
c      write(*,*)'mpsparse7',nel,lij,ilij,istk(ilij),istk(ilij+1)
...
c     
      ilv=iadr(lstk(top))
      if(istk(ilv).ne.257.and.istk(ilv).ne.4) then
         call funnam(ids(1,pt+1),'%talg_sparse',iadr(lstk(top)))
         fun=-1
         top=tops
         return
      ...
      if(istk(ilv).eq.257) then
         ...
         nel0=nel
         if(itv.eq.0) then
            call mpdij2sp(m,n,nel,istk(ilij),stk(lv),
     $           istk(lind),mm+nel0,istk(liw),ierr)
         else
            call mpwij2sp(m,n,nel,istk(ilij),stk(lv),stk(lv+nel0),
     $           istk(lind),mm+nel0,istk(liw),ierr)
         endif
         if(ierr.eq.2) then
