...
c     talg_f_mps mps_f_talg s_f_mps mps_f_s amax mps_s
      goto(78,   78,   78,  78,  213, 8) op-31
c
...
c       check dimensions
      ...
      if(istk(il1).ne.261.or.istk(il2).ne.261) then
...
c     $        ,stk(lwww)
         ...
         istk(il1+3)=0
         l1=sadr(il1+4)
         call unsfdcopy(m1,stk(lc),1,stk(l1),1)
c         write(*,*)stk(l1)
         l5=l1+m1
         lstk(top+1)=l5
c
...
c
         call unsfdcopy(m1,stk(lww),1,stk(l5),1)
c
c         write(*,*)stk(l5)
c
         l6=l5+m1
...
c
         ...
         istk(il6+3)=0
         l6=sadr(il6+4)
         call unsfdcopy(m1,stk(lwww),1,stk(l6),1)
c
c         write(*,*)stk(l6)
c
         top=top+1
...
c     sparse x full
     ...
     $              stk(l2),m2,stk(lc),m1,PCZERO)
         ...
         istk(il1)=algtype2(algcode(istk(il1)),1)
         istk(il1+1)=m1
         istk(il1+2)=n2
         istk(il1+3)=itr
         l1=sadr(il1+4)
...
c     cst*a
     ...
     $ .and.(si.eq.PCZERO)) then
         if(typecode(istk(il2)).eq.1) then
            istk(il1)=algtype2(algcode(istk(il2)),1)
            istk(il1+1)=m2
            istk(il1+2)=n2
            istk(il1+3)=0
            l1=sadr(il1+4)
            ...
            istk(il1)=algtype2(algcode(istk(il2)),5)
            istk(il1+1)=m2
            istk(il1+2)=n2
            istk(il1+3)=0
            istk(il1+4)=0
...
c     division a droite
 ...
 172  continue
     ...
     $ m1,PCZERO)
      ...
      istk(il1+3)=itr
      l1=sadr(il1+4)
      call unsfdcopy(m1*m2*(itr+1),stk(ld),1,stk(l1),1)
...
c
     ...
     $        stk(lat),stk(lat+nel2),istk(iat),istk(irc))
      ...
      call unsfdcopy(nel2*(it2+1),stk(lat),1,stk(l2),1)
...
c     call unsfdcopy(nel1*(it1+1),stk(l1),1,stk(ls),1)
c     istk(il1)=algtype2(algcode(istk(il1)),1)
c     l1=sadr(il1+4)
c
c     if(it1.eq.0) then
...
c     .  arg2=[]
         il1=iadr(lstk(top))
c         istk(il1)=algtype2(PCALGCODE,1)
          istk(il1)=1
        istk(il1+1)=0
         istk(il1+2)=0
         istk(il1+3)=0
...
c      write(*,*)' comp 2'
     ...
     $           nel,istk(irc),ierr,it1,it2)
            ...
            call mpdsosp(op,m1,n1,stk(l1),m2,n2,stk(l2),nel2,
     ...
     $           nel,istk(irc),ierr,it1,it2)
            ...
            call mpdspos(op,m1,n1,stk(l1),nel1,istk(irc1),
     $           m2,n2,stk(l2),nel,istk(irc),ierr)
         endif
      else
c      write(*,*) 'comp '       
         if(itr.eq.1) then
             call mpwsposp(op,m1,n1,stk(l1),stk(l1+nel1),nel1,
     $           istk(irc1),m2,n2,stk(l2),stk(l2+nel2),nel2,istk(irc2),
     $           nel,istk(irc),ierr,it1,it2)
         else
c       write(*,*) 'comp 5'      
            call mpdsposp(op,m1,n1,stk(l1),nel1,istk(irc1),
     $           m2,n2,stk(l2),nel2,istk(irc2),
     $           nel,istk(irc),ierr)
         endif
      endif
