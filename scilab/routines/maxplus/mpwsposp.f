      subroutine mpwsposp(op,ma,na,ar,ai,nela,inda,mb,nb,br,bi,nelb,
     $    indb, nelc,indc,ierr,ita,itb)
c    CopyWright INRIA
c!purpose
...
c     
      ...
      double precision tr,ti,tai,tbi,ZERO
      logical wcompa,z,z1
      external wcompa,commons
      call commons(ZERO)
c     
      nr=max(ma,mb)
      nc=max(na,nb)
...
c     compare all elements of a with scalar b  
 ...
 20      continue 
         ...
         z=wcompa(ZERO,ZERO,ZERO,ZERO,op)   
         do 30 i=1,nr
            indc(i)=0
            nira=inda(i)
            nirb=indb(i)
 ...
 21                  continue
                  ...
                  do 22 j=1,nc
                     if(j2.eq.j) then
                        if(itb.eq.0) then
                           z1=wcompa(ZERO,ZERO,br(jb),ZERO,op)
                        else
                           z1=wcompa(ZERO,ZERO,br(jb),bi(jb),op)
                        endif
                        if (z1) then
                           if(jc+1.gt.nelmx) goto 99
                           indc(nr+jc)=j
 ...
 22               continue
               ...
               if(nirb.eq.0) then
                  ...
                  do 23 j=1,nc
                     if(j1.eq.j) then
                        if(ita.eq.0) then
                           z1=wcompa(ar(ja),ZERO,ZERO,ZERO,op)
                        else
                           z1=wcompa(ar(ja),ai(ja),ZERO,ZERO,op)
                        endif
                        if (z1) then
                           if(jc+1.gt.nelmx) goto 99
                           indc(nr+jc)=j
