...
extern int C2F(cdftnc) __PARAMS((int *,double *,double *,double *,double *,double *, int *,double *));
...
static void cdftncErr __PARAMS((int status,double bound));
...
int cdftncI(char* fname,unsigned long l)
  
{ 
  int minrhs = 3,maxrhs = 5,minlhs=1,maxlhs=2,m1,n1,l1;
  Nbvars = 0;
  CheckRhs(minrhs,maxrhs);
  CheckLhs(minlhs,maxlhs);
  GetRhsVar(1, "c", &m1, &n1, &l1);
  if ( strcmp(cstk(l1),"PQ")==0) 
    {
      static int callpos[5] = {3,4,0,1,2};
      CdfBase(fname,3,2,callpos,"PQ","T,Df and Pnonc",1,C2F(cdftnc),
	       cdftncErr);
    }
  else if ( strcmp(cstk(l1),"T")==0) 
    {
      static int callpos[5] = {2,3,4,0,1};
      CdfBase(fname,4,1,callpos,"T","Df, Pnonc, P and Q",2,C2F(cdftnc),
	       cdftncErr);
    }
  else if ( strcmp(cstk(l1),"Df")==0) 
    {
      static int callpos[5] = {1,2,3,4,0};
      CdfBase(fname,4,1,callpos,"Df","Pnonc, P,Q, T",3,C2F(cdftnc),
	       cdftncErr);
    }
  else if ( strcmp(cstk(l1),"Pnonc")==0) 
    {
      static int callpos[5] = {0,1,2,3,4};
      CdfBase(fname,4,1,callpos,"Pnonc","P,Q, T and Df",4,C2F(cdftnc),
	       cdftncErr);
    }
  else 
    {
...
static void cdftncErr(status,bound)
     int status;
     double bound;
{
  static char *param[7]={"X", "P","Q","T","Df","Pnonc"};
  switch ( status ) 
    {
    case 1 : Scierror(999,"answer appears to be lower than lowest search bound %f\r\n",bound);break;
    case 2 : Scierror(999,"answer appears to be higher than greatest search bound %f\r\n",bound);break;
