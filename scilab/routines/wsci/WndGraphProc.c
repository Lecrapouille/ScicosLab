...
static BOOL ON_WND_GRAPH_WM_SETFOCUS(HWND hwnd, HWND hwndOldFocus);
static BOOL ON_WND_GRAPH_WM_CLOSE(HWND hwnd);
static BOOL ON_WND_GRAPH_WM_DESTROY(HWND hwnd);
static BOOL ON_WND_GRAPH_WM_DROPFILES(HWND hwnd,HDROP hDrop);
static void ON_WND_GRAPH_WM_VSCROLL(HWND hwnd, HWND hwndCtl, UINT code, int pos);
static void ON_WND_GRAPH_WM_HSCROLL(HWND hwnd, HWND hwndCtl, UINT code, int pos);
static BOOL ON_WND_GRAPH_WM_SIZE(HWND hwnd,UINT state,int cx,int cy);
static BOOL ON_WND_GRAPH_WM_PAINT(HWND hwnd);
static BOOL ON_WND_GRAPH_WM_CREATE(HWND hwnd,LPCREATESTRUCT lpCreateStruct);
static void ON_WND_GRAPH_WM_KEY(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags);
static BOOL ON_WND_GRAPH_WM_COMMAND(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify);
...
static BOOL ON_WND_GRAPH_WM_SIZE(HWND hwnd,UINT state,int cx,int cy)
{
  ...
  if (ScilabGC->CurResizeStatus == 0) {
    /* sciprint("horzsi.nPage=%d\n",horzsi.nPage);
     * sciprint("vertsi.nPage=%d\n",vertsi.nPage);
     * sciprint("cx=%d\n",cx);
     * sciprint("cy=%d\n",cy);
     * sciprint("horzsi.nMax=%d\n",horzsi.nMax);
     * sciprint("vertsi.nMax=%d\n",vertsi.nMax);
     */
    horzsi.nPage = cx;
    vertsi.nPage = cy;
    if(horzsi.nMax<cx) {
...
static BOOL ON_WND_GRAPH_WM_SETFOCUS(HWND hwnd, HWND hwndOldFocus)
{
  ...
  /* ajout d'une fonction pour forcer le focus
   * des fenetres tk
   */
  TkTakeFocus();
