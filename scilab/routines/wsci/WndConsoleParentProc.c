...
BOOL ON_WND_CONSOLE_WM_MOVE(HWND hwnd, int x, int y);
/*-----------------------------------------------------------------------------------*/
#if ((_MSC_VER >= 1200) && (_MSC_VER < 1300)) || defined(__MINGW32__) /* Visual Studio 6 */
  ...
  switch (message)
    {
		...
		HANDLE_MSG(hwnd,WM_MOVE,ON_WND_CONSOLE_WM_MOVE);
	}
...
BOOL ON_WND_CONSOLE_WM_CREATE(HWND hwnd,LPCREATESTRUCT lpCreateStruct)
{
	...
	/* Renomme la fenetre avec VERSION et numero x associé à la console*/  
	{
		...
		wsprintf(ScilexWindowName,"%s (%s",SCILAB_VERSION,SecondOccurence);
