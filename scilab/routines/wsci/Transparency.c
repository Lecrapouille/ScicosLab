...
SLWA_FUNC SetLayeredWindowAttributes_;
...
BOOL ActivateTransparencyMode(HWND hWnd)
{
  ...
  if ( (OS!=OS_WIN32_WINDOWS_NT_3_51) &&
      ...
      if (LoadUser32DLL==FALSE)
	{
      ...
      SetLayeredWindowAttributes_ = (SLWA_FUNC)GetProcAddress(hUser32, "SetLayeredWindowAttributes");
      TransparencyON=TRUE;
      bOK=TRUE;
    }
  return bOK;
...
void SetCurrentAlphaLevel(HWND hWnd ,int Alpha)
{
  ...
  if ( (OS!=OS_WIN32_WINDOWS_NT_3_51) &&
      ...
      if ( (Alpha>=0) && (Alpha<=255) )
	{
	  ...
	  SetLayeredWindowAttributes_(hWnd, (COLORREF)NULL,CurrentAlphaLevel, LWA_ALPHA);
	}
