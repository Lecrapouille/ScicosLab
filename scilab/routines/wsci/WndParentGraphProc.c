...
static BOOL ON_WND_PARENTGRAPH_WM_CLOSE(HWND hwnd);
static BOOL ON_WND_PARENTGRAPH_WM_DESTROY(HWND hwnd);
static BOOL ON_WND_PARENTGRAPH_WM_CREATE(HWND hwnd,LPCREATESTRUCT lpCreateStruct);
static BOOL ON_WND_PARENTGRAPH_WM_PAINT(HWND hwnd);
static BOOL ON_WND_PARENTGRAPH_WM_DROPFILES(HWND hwnd,HDROP hDrop);
static BOOL ON_WND_PARENTGRAPH_WM_COMMAND(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify);
static BOOL ON_WND_PARENTGRAPH_WM_SIZE(HWND hwnd,UINT state,int cx,int cy);
static void ON_WND_PARENTGRAPH_WM_EXITSIZEMOVE(HWND hwnd);
static void ON_WND_PARENTGRAPH_WM_ENTERSIZEMOVE(HWND hwnd);
static BOOL ON_WND_PARENTGRAPH_WM_GETMINMAXINFO(HWND hwnd, LPMINMAXINFO lpMinMaxInfo);
static void ON_WND_PARENTGRAPH_WM_CHAR(HWND hwnd, TCHAR ch, int cRepeat);
static void ON_WND_PARENTGRAPH_WM_KEY(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags);
static BOOL ON_WND_PARENTGRAPH_WM_SETFOCUS(HWND hwnd, HWND hwndOldFocus);
static BOOL ON_WND_PARENTGRAPH_WM_SYSCOMMAND(HWND hwnd, WPARAM wParam, LPARAM lParam);
static void ON_WND_PARENTGRAPH_WM_KILLFOCUS(HWND hwnd, HWND hwndNewFocus);
static BOOL ON_WND_PARENTGRAPH_WM_MOVE(HWND hwnd, int x, int y);
...
EXPORT LRESULT CALLBACK WndParentGraphProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  switch (message)
    ...
    case WM_SYSCOMMAND    : ON_WND_PARENTGRAPH_WM_SYSCOMMAND(hwnd,wParam,lParam); break;
      ...
      HANDLE_MSG(hwnd,WM_MOVE,ON_WND_PARENTGRAPH_WM_MOVE);
    }
...
static void ON_WND_PARENTGRAPH_WM_KEY(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags)
{
  struct BCG *ScilabGC = (struct BCG *) GetWindowLong (hwnd, 0);
  if ( fDown)
    SendMessage (ScilabGC->CWindow, WM_KEYDOWN,(WPARAM)(UINT)(vk),
		 MAKELPARAM((cRepeat), (flags)));
  else
    SendMessage (ScilabGC->CWindow, WM_KEYUP,(WPARAM)(UINT)(vk),
		 MAKELPARAM((cRepeat), (flags)));
}
