...
extern int check_pointer_win(int *x1,int *y1,int *win);
...
{
  int v_flag = 1,  num = *number;
  if (sci_exists_winid(num) != TRUE ) return 0;
  ...
  /* destroy ScilabXgc (set to NULL) if it is the last window in the list
   * or reset ScilabXgc to the next one.
   */
  DeleteSGWin (num);
  /* That's why we can not use version_flag below because this function uses
   * ScilabXgc->graphicsversion that could have been possibly previously
   * deleted
