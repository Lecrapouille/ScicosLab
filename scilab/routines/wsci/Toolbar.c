...
static void CreateButtonToolBar(struct BCG * ScilabGC,char *Name,
				HMENU CallbackID,int ID_ICON,
				char *ToolTipString,DWORD Style);
...
void CreateGraphToolBar(struct BCG * ScilabGC) 
{
  ...
  CreateButtonToolBar(ScilabGC,MSG_SCIMSG14,
		      (HMENU)TOOLBAR_ZOOM,IDI_ZOOM,MSG_SCIMSG14,
		      WS_CHILD|WS_VISIBLE|BS_ICON);
  CreateButtonToolBar(ScilabGC,MSG_SCIMSG15,
		      (HMENU)TOOLBAR_UNZOOM,IDI_UNZOOM,MSG_SCIMSG15,
		      WS_CHILD|WS_VISIBLE|BS_ICON);
  CreateButtonToolBar(ScilabGC,MSG_SCIMSG16,
		      (HMENU)TOOLBAR_ROTATE3D,IDI_3DROT,MSG_SCIMSG16,
		      WS_CHILD|WS_VISIBLE|BS_ICON);
  CreateButtonToolBar(ScilabGC,MSG_SCIMSG106,
		      (HMENU)TOOLBAR_GED,IDI_GED,MSG_SCIMSG106,
		      WS_CHILD|WS_VISIBLE|BS_ICON);
  CreateButtonToolBar(ScilabGC,MSG_SCIMSG113,
		      (HMENU)TOOLBAR_PICKER,IDI_PICKER,MSG_SCIMSG113,
		      WS_CHILD|WS_VISIBLE|BS_ICON|BS_AUTOCHECKBOX|BS_PUSHLIKE);

  ScilabGC->lpmw.LockToolBar=FALSE;
  if (DefaultShowToolBar == FALSE) 
    {
...
static void ShowToolBar(LPTW lptw)
{
  int i=0;	      		
  LPMW lpmw;      		
  RECT rect;
...
static void HideToolBar(LPTW lptw)
{
  int i=0;	      		
  LPMW lpmw;      		
  RECT rect;
...
int HideToolBarWin32(int WinNum)
{
  if (WinNum == -1)
    ...
    {
      struct BCG *ScilabGC= GetWindowXgcNumber (WinNum);
...
static void CreateButtonToolBar(struct BCG * ScilabGC,char *Name,HMENU CallbackID,
				int ID_ICON,char *ToolTipString,DWORD Style)
{
  HICON IconButton;
  ScilabGC->lpmw.hButton[ScilabGC->lpmw.nButton]= 
    CreateWindow("button",Name,Style,
		 ButtonToolBarWeight*ScilabGC->lpmw.nButton, 0,
		 ButtonToolBarWeight, ToolBarHeight,
		 ScilabGC->CWindow,(HMENU) CallbackID,
		 ScilabGC->lpgw->hInstance, NULL);
  IconButton=(HICON)LoadImage( GetModuleHandle(MSG_SCIMSG9), (LPCSTR)ID_ICON,IMAGE_ICON,
			       ButtonToolBarWeight,ToolBarHeight,LR_DEFAULTCOLOR);
  SendMessage(ScilabGC->lpmw.hButton[ScilabGC->lpmw.nButton],
	      BM_SETIMAGE, IMAGE_ICON, (LPARAM)IconButton);
