...
/*************************************************
 * generate a virtual event for tk widget identified by the
 * tcl variable $w in the main scilab tcl interpreter
 *************************************************/
void TkTakeFocus(void)
{
#ifdef WITH_TK
  int RET;
  if (TK_Started==1) {
    if (TCLinterp!=NULL) {
      RET=Tcl_Eval(TCLinterp,"event generate $w <<GraphTakeFocus>>");
      RET=Tcl_Eval(TCLinterp,"catch {event generate .palettes <<GraphTakeFocus>>}");
    }
  }
#endif
}
