...
static BOOL CALLBACK FilesAssociationDlgProc (HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam);
static BOOL ON_FILESASSOCIATIONBOX_WM_INITDIALOG(HWND hDlg,HWND hwndFocus, LPARAM lParam);
static BOOL ON_FILESASSOCIATIONBOX_WM_COMMAND(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify);
static void ON_FILESASSOCIATIONBOX_WM_CLOSE(HWND hwnd);
...
EXPORT BOOL CALLBACK FilesAssociationDlgProc (HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam);
...
EXPORT void WINAPI FilesAssociationBox (HWND hwnd)
{
  DLGPROC lpfnFilesAssociationDlgProc = (DLGPROC) MyGetProcAddress("FilesAssociationDlgProc",FilesAssociationDlgProc);
  if ( lpfnFilesAssociationDlgProc == NULL ) 
    MessageBox(NULL,"FilesAssociationDlgProc not found in Libscilab.dll ","Warning",MB_ICONERROR); 
  DialogBox(hdllInstance,(LPCSTR)IDD_FILEASSOCIATION,hwnd,FilesAssociationDlgProc) ;
}
...
BOOL UpdateSCE(void)
{
	...
	{
		case 1:
			wsprintf(KeyRegistryWithLanguage,"SCE_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

		default :case 0:
			wsprintf(KeyRegistryWithLanguage,"SCE_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
...
BOOL UpdateSCI(void)
{
	...
	{
	case 1:
		wsprintf(KeyRegistryWithLanguage,"SCI_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

	default :case 0:
		wsprintf(KeyRegistryWithLanguage,"SCI_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
	if (bOK) bOK=UpdateAKey(KeyRegistryWithLanguage,Line);
...
BOOL UpdateTST(void)
{
	...
	{
	case 1:
		wsprintf(KeyRegistryWithLanguage,"TST_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

	default :case 0:
		wsprintf(KeyRegistryWithLanguage,"TST_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
	if (bOK) bOK=UpdateAKey(KeyRegistryWithLanguage,Line);
...
BOOL UpdateDEM(void)
{
	...
	{
	case 1:
		wsprintf(KeyRegistryWithLanguage,"DEM_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

	default :case 0:
		wsprintf(KeyRegistryWithLanguage,"DEM_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
	if (bOK) bOK=UpdateAKey(KeyRegistryWithLanguage,Line);
...
BOOL UpdateCOS(void)
{
	...
	{
	case 1:
		wsprintf(KeyRegistryWithLanguage,"COS_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

	default :case 0:
		wsprintf(KeyRegistryWithLanguage,"COS_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
	if (bOK) bOK=UpdateAKey(KeyRegistryWithLanguage,Line);
...
BOOL UpdateCOSF(void)
{
	...
	{
	case 1:
		wsprintf(KeyRegistryWithLanguage,"COSF_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
		break;

	default :case 0:
		wsprintf(KeyRegistryWithLanguage,"COSF_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
		break;
	}
	if (bOK) bOK=UpdateAKey(KeyRegistryWithLanguage,Line);
...
void InitCheckSCI(HWND hDlgParent)
{
	BOOL bOK[5];
	char Key[L_PATH_MAX];
	char KeyENG[L_PATH_MAX];
	char KeyFR[L_PATH_MAX];
	char GoodValue[L_PATH_MAX];
	char *path=GetWhereIsThisExe();
	...
	wsprintf(KeyFR,"SCI_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"SCI_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
...
void InitCheckSCE(HWND hDlgParent)
{
	...
	wsprintf(KeyFR,"SCE_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"SCE_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
	wsprintf(GoodValue,"\"%s%s\" -X \"%%1\"",path,WSCILEX);
	bOK[3]=TestRegistryKey(KeyENG,GoodValue);
	if (!bOK[3]) bOK[3]=TestRegistryKey(KeyFR,GoodValue);
...
void InitCheckTST(HWND hDlgParent)
{
	...
	wsprintf(KeyFR,"TST_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"TST_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
	wsprintf(GoodValue,"\"%s%s\" -X \"%%1\"",path,WSCILEX);
	bOK[3]=TestRegistryKey(KeyENG,GoodValue);
	if (!bOK[3]) bOK[3]=TestRegistryKey(KeyFR,GoodValue);
...
void InitCheckDEM(HWND hDlgParent)
{
	...
	wsprintf(KeyFR,"DEM_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"DEM_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
	wsprintf(GoodValue,"\"%s%s\" -X \"%%1\"",path,WSCILEX);
	bOK[3]=TestRegistryKey(KeyENG,GoodValue);
	if (!bOK[3]) bOK[3]=TestRegistryKey(KeyFR,GoodValue);
...
void InitCheckCOS(HWND hDlgParent)
{
	...
	wsprintf(KeyFR,"COS_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"COS_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
	wsprintf(GoodValue,"\"%s%s\" -X \"%%1\"",path,WSCILEX);
...
void InitCheckCOSF(HWND hDlgParent)
{
	...
	wsprintf(KeyFR,"COSF_scilab_file\\shell\\Executer avec %s\\command",SCILAB_VERSION);
	wsprintf(KeyENG,"COSF_scilab_file\\shell\\Run with %s\\command",SCILAB_VERSION);
	wsprintf(GoodValue,"\"%s%s\" -X \"%%1\"",path,WSCILEX);
	bOK[3]=TestRegistryKey(KeyENG,GoodValue);
	if (!bOK[3]) bOK[3]=TestRegistryKey(KeyFR,GoodValue);
