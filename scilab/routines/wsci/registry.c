...
static BOOL WindowsQueryRegistryNumberOfElementsInList(char *ParamIn1,char *ParamIn2,int *Number);
static BOOL WindowsQueryRegistry(char *ParamIn1,char *ParamIn2,char *ParamIn3,char *ParamOut1,int *ParamOut2,
				 BOOL *OuputIsREG_SZ);
static HKEY GetHkeyrootFromString(char *string);
static BOOL WindowsQueryRegistryList(char *ParamIn1,char *ParamIn2,int dimMax,char **ListKeys);

int InterfaceWindowsQueryRegistry _PARAMS((char *fname))
{
  int l1,n1,m1, paramoutINT;
  char *param1=NULL,*param2=NULL,*param3=NULL, *output=NULL ;
  BOOL OuputIsREG_SZ, TestWinQuery=FALSE;
...
void ReadRegistryTxt (LPTW lptw)
{
   ...
   *   HKEY_CURRENT_USER\\SOFTWARE\\ScicosLab\\"VERSION"\\Settings
...
/* Modification Sauvegarde dans la base de registre dans 
 *     HKEY_CURRENT_USER\\SOFTWARE\\ScicosLab\\"VERSION"\\Graph Settings
 *  "Version" correspondant à la version de Scilab
 *  Sauvegarde dans HKEY_CURRENT_USER car données dépendant de l'utilisateur
 */
...
void WriteRegistryGraph (struct BCG *ScilabGC)
{
  ...
  wsprintf(Clef,"SOFTWARE\\ScicosLab\\%s\\Graph Settings",SCILAB_VERSION);  	
  RegCreateKeyEx(HKEY_CURRENT_USER, Clef, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, &result);
