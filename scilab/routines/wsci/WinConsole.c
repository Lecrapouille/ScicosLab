...
void RenameConsole(void)
{
	...
	{
		wsprintf(ScilexConsoleName,"%s %s",NameConsole,SCILAB_VERSION);
...
void CreateScilabConsole(int ShowBanner)
{
	...
	Windows_Console_State=0; /* Console DOS Cachee par defaut */
	AllocConsole();

	Current_Number_of_Scilex=FindFreeScilexNumber();
	wsprintf(ScilexConsoleName,"%s %s (%d)",NameConsole,SCILAB_VERSION,Current_Number_of_Scilex);
	SetConsoleTitle(ScilexConsoleName);
	...
	if (ShowBanner)
	{
		...
		printf(line);
                wsprintf(line,"                         %s\r\n\n",SCILAB_VERSION);
		printf(line);
		strcpy(line,"                  Copyright (c) 1989-2011          \r\n");
		printf(line);
		strcpy(line,"                         (INRIA, ENPC)      \r\n");
		printf(line);
                strcpy(line,"        ___________________________________________\r\n\r\n");
		printf(line);
	}
...
int FindFreeScilexNumber(void)
{
	...
	wsprintf(NameScilex,"%s (%d)",SCILAB_VERSION,Number_of_Scilex);
