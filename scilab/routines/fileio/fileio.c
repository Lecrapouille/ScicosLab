...
int int_objsscanf(char *fname,unsigned long fname_len)
{
  ...
  lw = iarg + Top_stack - Rhs; /* Scilab string vector */
  if (! C2F(getwsmat)("sscanf",&Top_stack,&lw,&m1,&n1,&il1,&ild1,6L)) return 0;
...
static int  GetString(char *fname, int *previous_t, int *arg, int narg, int *ic, int ir, char **sval)
{
  ...
  lw = *arg + Top_stack - Rhs;
  
  if (! C2F(getwsmat)(fname,&Top_stack,&lw,&mx,&nx,&il,&ild, strlen(fname))) return RET_BUG;
  else {
    if ( *ic>nx ) {
      *arg=*arg+1;
