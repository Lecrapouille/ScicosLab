...
int OLE_convert_header( struct OLE_object *ole )
{
  ...
  h->minor_version = get_2byte_value((char *)header_minor_version(hb));
  h->dll_version = get_2byte_value((char *)header_dll_version(hb));
  h->byte_order = get_2byte_value((char *)header_byte_order(hb)); /** 0xFEFF = Little endian, 0xFFFE = big endian **/
  h->sector_shift = get_2byte_value((char *)header_sector_shift(hb));
  h->sector_size = 1 << h->sector_shift;

  h->mini_sector_shift = get_2byte_value((char *)header_mini_sector_shift(hb));
  h->mini_sector_size = 1 << h->mini_sector_shift;

  h->fat_sector_count = get_4byte_value((char *)header_fat_sector_count(hb)); /** Total number of sectors use for the SAT **/
  h->directory_stream_start_sector = get_4byte_value((char *)header_directory_stream_start_sector(hb)); /** Start sector-ID for the DIRECTORY STREAM **/
  h->mini_cutoff_size = get_4byte_value((char *)header_mini_cutoff_size(hb));
  h->mini_fat_start = get_4byte_value((char *)header_mini_fat_start(hb));
  h->mini_fat_sector_count = get_4byte_value((char *)header_mini_fat_sector_count(hb));
  h->dif_start_sector = get_4byte_value((char *)header_dif_start_sector(hb));
  h->dif_sector_count = get_4byte_value((char *)header_dif_sector_count(hb));
  ...
  for (i = 0; i < h->fat_sector_count; i++)
    {
      ...
      h->FAT[i] = get_4byte_value((char *)fat_start +(LEN_ULONG *i));
    }
...
int OLE_convert_directory( struct OLE_object *ole, unsigned char *buf, struct OLE_directory_entry *dir )
{
   ...
   ** (8+1)*2 = 18 bytes
   **/
  dir->element_name_byte_count = get_2byte_value((char *) (buf + 0x40) );
  ...
  dir->stream_size = get_4byte_value((char *) (buf +0x78) );
      ...
      if (ole->header.dif_sector_count > 0)
	{
	  ...
	  for (i = 0; i < ole->header.dif_sector_count; i++)
	      ...
	      if (getblock_result != OLE_OK)
		{
	      ...
	      do {
		import_sector = get_4byte_value((char *) DIF );
		...
		{
		  current_sector = get_4byte_value((char *) fat_block_end );
...
int OLE_follow_chain( struct OLE_object *ole, int FAT_sector_start )
{
  ...
  do {
    ...
    next_sector = get_4byte_value((char *) next_sector_location );
...
int OLE_follow_minichain( struct OLE_object *ole, int miniFAT_sector_start )
{
  ...
  do {
    ...
    */
    ...
    next_sector = get_4byte_value((char *) ole->miniFAT +(LEN_ULONG *current_sector));
  ...
  if (chain_length > 0)
      ...
      if (buffer != NULL)
	{
	  ...
	  do {
	    ...
	    next_sector = get_4byte_value((char *) ole->FAT +(LEN_ULONG *current_sector));
	    current_sector = next_sector;
	    tick++;
  ...
  while (current_property < property_limit)
    {
      ...
      property_value = get_1byte_value((char *) current_property);
