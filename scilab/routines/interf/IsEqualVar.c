...
{
  int topk, top1, srhs, k,kmin, l, res=0;
  int one = 1;
  int l1,lk, il1,ilk;
  ...
  if (Rstk[Pt]==914||Rstk[Pt]==915) { /* coming back after evaluation of overloading function */
    ...
    srhs = Ids[2 + Pt * nsiz];
    topk = Ids[3 + Pt * nsiz];
    top1 = Top_stack-1-srhs+1;/* Top-1 because Top has been increased to store the result of overloading function */
  }
  else {
    CheckRhs(2,2000000);
    CheckLhs(1,1);
    srhs = Rhs;
    top1 = Top_stack-srhs+1;
    topk = top1 + 1;
    kmin = 2;
    MaxRec = 0;
    Rrec = NULL;
...
static int IsEqualOverloaded(double *d1, int n1, double *d2, int n2)
{
  ...
  *Lstk(Top_stack+1)=*Lstk(Top_stack)+2;
  ...
  /*DEBUG_OVERLOADING("IsEqualVar Overloaded calls the parser Top_stack=%d, Rhs=%d, Pt=%d\n",Top_stack,Rhs,Pt);*/
...
static int IsEqualDoubleSparseMat(double *d1, double *d2) /* a faire */
{
  int l,nel;
  int *id1 = (int *) d1;
  int *id2 = (int *) d2;
...
static int IsEqualMatlabSparseMat(double *d1, double *d2) /* a faire */
{
  int l,nel;
  int *id1 = (int *) d1;
  int *id2 = (int *) d2;
...
static int IsEqualBoolSparseMat(double *d1, double *d2) /* a faire */
{
  int l,nel;
  int *id1 = (int *) d1;
  int *id2 = (int *) d2;
...
{
  int i;
  /* unsigned int dl = sizeof(double); */
  /* unsigned char *l1 = (unsigned char *)d1; */
  /* unsigned char *l2 = (unsigned char *)d2; */
  ...
  for (i = 0; i<n; i++)
    {
      ...
      if ( ISNAN(d1[i]) )
	{
	  if ( ! ISNAN(d2[i])) goto DIFFER;
	}
      else
	{
	  if (ISNAN(d2[i])) goto DIFFER;
	  if ( d2[i]!= d1[i]) goto DIFFER;
	}
#else
      int j;
      for (j=0 ; j < dl ; j++)
	if (l1[j+ i*dl] != l2[j+i*dl]) goto DIFFER;
#endif
    }
  // fprintf(stderr,"IsEqualDoubleArrayBinary are equals %f %f\n",d1[0],d2[0]);
  return 1;
 DIFFER:
  // fprintf(stderr,"IsEqualDoubleArrayBinary differs %f %f\n",d1[0],d2[0]);
  return 0;
}
