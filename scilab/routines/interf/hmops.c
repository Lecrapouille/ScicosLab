...
{
  int il;
  il = iadr(*Lstk( num + Top_stack - Rhs ));
  return(*istk(il+1));
}
...
int C2F(ishm)()
{
  /* teste si l'argument en Top_stack est une hypermatrice */
  int il, il1, il2;
			       ...
			       int nmax, int *ind_max)
{
  ...
  switch ( GetType(pos) )
    ...
    case (SCI_POLYNOMIAL):
      ...
      il = iadr( *Lstk( pos + Top_stack - Rhs ) );
      ...
      else
        {
          ...
          *mn = abs(ifin-ideb)/abs(ipas)+1;
          if ( ipas > 0 )
            ...
            {
              *ind_max = ideb; ind_min = ideb  + (*mn-1)*ipas;
            }
 ...
 */
 the_end:
  il = iadr(*Lstk(Top_stack));
  topk = *istk(il + 2);
  Top_stack = Top_stack - Rhs + 1;
  ilp = iadr(*Lstk(Top_stack));
  *istk(ilp) = -1;
  *istk(ilp+1) = -1;
  *istk(ilp+2) = topk;
  if ( topk > 0 )
