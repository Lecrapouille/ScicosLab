     ...
     integer *lw, *m, *n, *nzMax, *it;
{
  ...
  *Lstk(*lw +1) = sadr(ix1) + (*it + 1) * NZMAX + 1;
  ...
  C2F(intersci).ntypes[*lw-Top_stack+Rhs-1] = '$';
  C2F(intersci).iwhere[*lw-Top_stack+Rhs-1] = *Lstk(*lw);
  /* C2F(intersci).lad[*lw-Top_stack+Rhs-1] = should point to numeric data */
  return TRUE_;
}
...
/*------------------------------------------------------------------ 
 * realmat : 
 *     Top_stack is supposed to be a matrix 
