...
{
  integer   ix1 = *m * *n;
  integer Rhs_k = Rhs , Top_k = Top_stack ;
  integer l4, id[nsiz], lc, lr;
  
  C2F(str2name)(namex, id, name_len);
  /* jpc april 2002 */ 
  /* ++Top_stack; */
  Top_stack = Top_stack + Nbvars + 1; 
  if (! C2F(cremat)("cwritemat", &Top_stack, &cx0, m, n, &lr, &lc, 9L)) return  FALSE_;
  /* printf("cwritemat %s ix1=%d cx1=%d Top_stack=%d\n",namex,ix1,cx1,Top_stack); */
...
int C2F(cwritecmat)(char *namex,integer *m, integer*n,double *mat,unsigned long name_len)
{
	...
	Top_stack = Top_stack + Nbvars + 1; 
	if (! C2F(cremat)("cwritecmat", &Top_stack, &IT, m, n, &lr, &lc, 10L)) return  FALSE_;
	C2F(dcopy)(&ix1, mat, &cx1, stk(lr ), &cx1);
	Rhs = 0;
	l4 = C2F(iop).lct[3];
	C2F(iop).lct[3] = -1;
     ...
     unsigned long name_len;
{
  ...
  C2F(str2name)(namex, id, name_len);
  Top_stack = *number + Top_stack -Rhs;
  Rhs = 0;
  l4 = C2F(iop).lct[3];
  C2F(iop).lct[3] = -1;
  C2F(stackp)(id, &cx0);
     ...
     unsigned long name_len,  chai_len;
{
    ...
    /* ++Top_stack; */
    Top_stack = Top_stack + Nbvars + 1; 
    if (! C2F(cresmat2)("cwritechain", &Top_stack, m, &lr, 11L)) {
	return FALSE_;
    }
     ...
     unsigned long name_len;
{
    ...
    *lp = *Lstk(Fin );
    if (*Infstk(Fin ) == 2) {
      *fin = Fin = *istk(iadr(*Lstk(Fin )) + 1 +1);
      *lp = *Lstk(Fin);
    }
    return  TRUE_;
}
...
int C2F(cwritebmat)(char *namex, integer *m, integer *n,  int *mat, unsigned long name_len)
{
	...
	C2F(str2name)(namex, id, name_len);
	Top_stack = Top_stack + Nbvars + 1; 
	if (! C2F(crebmat)("cwritebmat", &Top_stack, m, n, &lr, 10L)) return  FALSE_;
