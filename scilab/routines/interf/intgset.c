...
/*-----------------------------------------------------------------------------------*/
static int setticks(char * xyztick, sciPointObj* psubwin, int *ptrindex,char **ptrindexS, int * numrow, int * numcol);
...
{
  long int l3;
  integer m1,n1,l1,m2,n2,l2,numrow3,numcol3,num,cur,na,verb=0;
  unsigned long hdl;
  int lw,t2;
  BOOL vis_save;
  sciPointObj *pobj;
  ...
  if ( (hdl != (unsigned long)0) ) { /* F.Leray 16.03.04*/
	...
	GetListRhsVar(3,2,"d",&numrow[0],&numcol[0],&ptrindex[0]);
	    ...
	    int lv;
            ...
            GetListRhsVar(3,3,"d",&numrow[1],&numcol[1],&lv);
        ...
        else
        {
          ...
          GetListRhsVar(3,3,"S",&numrow[1],&numcol[1],&ptrindexS);
        ...
        if(setticks(cstk(l2),pobj, ptrindex,ptrindexS, numrow, numcol) != 0) return 0;
	...
	{
	  int l_l3=l3;
	  if (sciSet(pobj, cstk(l2), &l_l3, &numrow3, &numcol3)!=0) {
	    Scierror(999,"%s: %s\r\n",fname,error_message);
	    return 0;
	  }
	}
      ...
      {
	/* F.Leray 02.05.05 : main case (using sciGetPoint routine inside GetProperty.c) */
	int l_l3=l3;
	if (sciSet(pobj, cstk(l2), &l_l3, &numrow3, &numcol3)!=0) {
	  Scierror(999,"%s: %s\r\n",fname,error_message);
	  return 0;
    ...
    {
      int l_l3=l3;
      if (sciSet((sciPointObj *) NULL, cstk(l2), &l_l3, &numrow3, &numcol3)!=0) {
	Scierror(999,"%s: %s\r\n",fname,error_message);
	return 0;
      }
    }
...
static int setticks(char * xyztick, sciPointObj* psubwin, int * ptrindex,char **ptrindexS, int * numrow, int * numcol)
{
  ...
  if(strcmp(xyztick,"x_ticks")==0)
	...
	for(i=0;i<old_prod;i++) { /* we free the old vector components (size == old_prod) */
      ...
      if ( prod > 0 )
      {
        ppsubwin->axes.u_xlabels = ptrindexS;
      }
    }
	...
	for(i=0;i<old_prod;i++) {  /* we free the old vector components (size == old_prod) */
      ...
      if ( prod > 0 )
      {
        ppsubwin->axes.u_ylabels = ptrindexS;
      }
    }
	...
	for(i=0;i<old_prod;i++) { /* we free the old vector components (size == old_prod) */
      ...
      if ( prod > 0 )
      {
        ppsubwin->axes.u_zlabels = ptrindexS;
      }
    }
  return 0;
}
...
int sciSet(sciPointObj *pobj, char *marker, int *value, int *numrow, int *numcol)
{
  ...
  /********************** context graphique ******************************/
    ...
    if ( sciGetEntityType(pobj) == SCI_SURFACE)
      {
	tag = pSURFACE_FEATURE (pobj)->typeof3d==SCI_PLOT3D
	  ||  pSURFACE_FEATURE (pobj)->typeof3d==SCI_FAC3D;
      }
    if ( sciGetEntityType(pobj) == SCI_SURFACE && tag ){
      if (strcmp(cstk(*value),"on")==0 )
	sciSetIsLine((sciPointObj *) pobj,1);
      else if (strcmp(cstk(*value),"off")==0 )
	sciSetIsLine((sciPointObj *) pobj,0);
