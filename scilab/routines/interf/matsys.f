...
c
 ...
 201  continue
      top=topk-rhs
      if (imode.eq.0.and.num.eq.-1) then 
c        back to default values 
         errct=0
         errpt=0
         top=top+1
         call objvide('errcatch',top)
...
c
      ...
      if (mem.lt.l) then
         buf='Required memory too small for defined global data'
         call error(1503)
         return
      endif
...
c
      ...
      top=top-1
c
      if (stk(l).lt.1000.or.stk(l).gt.((2.0d0**31)-1)/8) then
...
c     Check if a compiled macro is running
 ...
 50   top=top+1
      ...
      subroutine intstacksizevoid 
c     A new version which uses fortran pointers
c     jpc December 2011 
      include '../stack.h'
      integer p, iadr,sadr
      logical checkrhs,checklhs,cremat,getscalar,eqid 
c     local variable 
c      double precision stkn(*)
c      pointer (stkptrn,stkn)
c
      iadr(l)=l+l-1
      sadr(l)=(l/2)+1
c
...
c
      ...
      if (rhs.eq.0) then
         top=top+1
         if(.not.cremat('stacksize',top,0,1,2,l,lc)) return
         stk(l)=lstk(isiz+1)-lstk(1)
         stk(l+1)=lstk(isiz+1)-lstk(bot)
         return
      endif
...
c      call stackshow()
      mem=stk(l)
      memold=lstk(isiz+1)-lstk(1)
      if (mem.eq.memold) goto 50
      lbot=lstk(isiz+1)-lstk(bot)
      ltop=0
      if (top.gt.0) ltop=lstk(top+1)-lstk(1)
      if (mem.lt.lbot+ltop) then
         buf='Required memory too small for defined data'
...
c      call stackshow()
 50   top=top+1
      ...
      subroutine setstacks(mem,memflag)
c     A new version which uses fortran pointers
c     jpc December 2011 
      include '../stack.h'
      integer p, iadr,sadr
      logical checkrhs,checklhs,cremat,getscalar,eqid 
c     local variable 
       double precision stkn(1)
c      double precision stkn(*)
c      pointer (stkptrn,stkn)
c
      iadr(l)=l+l-1
      sadr(l)=(l/2)+1
c     record stksizr in common and set new position of bot 
      ...
      call scimemp(stksizr+stksizg+1,stkptrn)
      if(stkptrn.eq.0) then
         call error(112)
         return
      endif
c     top position (should be one)
      l1=lstk(1)
c     copy the top variables of the stack to new position
      if (top.gt.0) then 
         ltop=lstk(top+1)-lstk(1)
         call unsfdcopy(ltop,stk(l1),1,stkn(l1),1)
      endif
c     no offset 
      kd1=0
c      do 30 k=1,top+1
c        lstk(k)=lstk(k)+kd1
c 30   continue
c     copy the bottom of the stack, l2 is the previous position
      l2=lstk(bot)
      lbot=lstk(isiz+1)-lstk(bot)
C     les nouvelles valeurs doivent etre 
C     lstk(isiz)=lstk(1)+ mem 
C     lskt(bot)= lstk(isiz+1) -lbot 
      
c     copy the bottom of the stack
      call unsfdcopy(lbot,stk(l2),1,stkn(l1+memr-lbot),1)
      kd2=l1+memr-lbot -l2 
c     update position variables following bot
      do 40 k=bot,isiz+1
         lstk(k)=lstk(k)+kd2
 40   continue
c     take care of global variables 
c     l is the size of global stack
      l=lstk(gtop+1)-lstk(isiz+2)
c     the new position of global should be 
c     lstk(isiz+2)= stksizr+1 
      call unsfdcopy(l,stk(lstk(isiz+2)),1,stkn(stksizr+2),1)
      kd=stksizr+2-lstk(isiz+2)
      do 05 k=isiz+2,gtop+1
         lstk(k)=lstk(k)+kd
 05   continue 
      lstk(gbot)=lstk(isiz+2)+stksizg-1
c     free previous stack
      call freememp(stkptr)
      stkptr = stkptrn
c     update reference variables
      do 41 i=1,top
         if(infstk(i).ne.2) then
...
c     .        variable j refers this global var
 ...
 09   continue
      ...
      write(06,*) 'pile size=[',stksizr,stksizg,stklpvar,']'
      write(06,*) 'var=[', lstk(bot),lstk(isiz),lstk(isiz+1),']'
      write(06,*) 'gvar=[', lstk(gtop+1),lstk(gbot),']'

      write(06,*) 'stack k=',k,'lstk(k)=',lstk(k),lstk(k+1)
      k=top+1
      write(06,*) 'stack k=',k,'lstk(k)=',lstk(k)
      do 441 k=bot,isiz
         call cvname(idstk(1,k),buf(1:nlgh),1)
         write(06,*) 'bottom',k,' ',
     &        buf(1:nlgh),'{',lstk(k),lstk(k+1),'}'
 441  continue
      do 06 k=isiz+2,gtop+1
         call cvname(idstk(1,k),buf(1:nlgh),1)
         write(06,*) 'global',k,' ',
     &        buf(1:nlgh),'{',lstk(k),lstk(k+1),'}'
 06   continue
...
c     update pointers in variables which refer this global var
         do 07 j=bot,isiz-1
            if(infstk(j).eq.2) then
               if(eqid(idstk(1,j),idstk(1,i))) then
c     .        variable j refers this global var
                  write(06,*) 'couple',j,i, lstk(j),lstk(i)
                  ilj=iadr(lstk(j))
                  write(06,*) '      ',ilj,istk(ilj+1) 
c                  istk(ilj+1)=lstk(i)
c                  istk(ilj+2)=i
               endif
            endif
