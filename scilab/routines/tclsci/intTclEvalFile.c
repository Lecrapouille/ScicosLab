...
int C2F(intTclEvalFile) (char *fname, unsigned long fname_len)
{
  FILE* tmpFile; 
  /* execute Tcl scripts */
  int m1,n1,l1;
  ...
  if (GetType(1) == sci_strings)
      ...
      if (RET==TCL_ERROR)
	{
	  if (C2F(iop).err>0) 
	    {
	      sciprint(TCL_MSG1,fname,TclGetErrorLine(TCLinterpreter),cstk(l1));
	    }
	  else
	    {
	      Scierror(999,TCL_MSG1,fname,TclGetErrorLine(TCLinterpreter),cstk(l1),Tcl_GetStringResult(TCLinterpreter));
	      return 0;
	    }
	}
    }
