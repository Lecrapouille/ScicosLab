...
int C2F(intmsparse)(integer *id)
{
    ...
    /* Function Body */
    Rhs = Max(0,Rhs);
    top0 = Top_stack + 1 - Rhs;
    tops = Top_stack;

    lw = C2F(vstk).lstk[Top_stack];
    if (Lhs != 1) {
	C2F(error)(&c41);
	return 0;
    }
    ...
    il = C2F(vstk).lstk[Top_stack-1] + C2F(vstk).lstk[Top_stack-1] - 1;
    if (*istk(il) == 5) {
	nel = *istk(il + 4);
	m = *istk(il+1);
	n = *istk(il + 2);
	...
	*istk(il ) = 7;
	...
	C2F(unsfdcopy)(&I1, stk(lat ), &c1, stk(lr ), &
		c1);
	C2F(vstk).lstk[Top_stack] = lr + nel * (it + 1);
    } else if (*istk(il ) == 7) {
    } else {
	C2F(error)(&c44);
	return 0;
...
int C2F(intmspget)(integer *id)
{
	...
	*istk(il + 3) = 0;
	I1 = il + 4;
	C2F(vstk).lstk[Top_stack] = I1 / 2 + 1;
	if (Lhs >= 2) {
	    ++Top_stack;
	    il = C2F(vstk).lstk[Top_stack -1] + C2F(vstk).lstk[Top_stack -1] - 
		    1;
	    *istk(il ) = 1;
	    *istk(il+1) = 0;
	    *istk(il + 2) = 0;
    ...
    *istk(il + 2) = 2;
    *istk(il + 3) = 0;
    C2F(vstk).lstk[Top_stack] = lij + (nel << 1);
    if (Lhs >= 2) {
      /*           V              */
	++Top_stack;
	il = C2F(vstk).lstk[Top_stack -1] + C2F(vstk).lstk[Top_stack -1] - 1;
	...
	*istk(il + 2) = 1;
	*istk(il + 3) = it;
	C2F(vstk).lstk[Top_stack] = lv + nel * (it + 1);
    }
    if (Lhs == 3) {
      /*            mn             */
	++Top_stack;
...
int C2F(intmfull)(integer *id)
{
    ...
    *istk(il ) = 1;
    ...
    C2F(vstk).lstk[Top_stack] = l + m * n * (it + 1);
    return 0;
}
