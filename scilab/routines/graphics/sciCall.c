...
extern int plot2dn(int ptype,char *logflags,double *x,double *y,int *n1,int *n2,int *style,
		   char *strflag,char *legend,double *brect,int *aaint, BOOL flagNax);

/* should be removed in function calls */
#define bsiz 4096
...
void Objplot2d (ptype,logflags,x,y,n1,n2,style,strflag,legend,brect,aaint,flagNax)
     ...
     BOOL flagNax;
{ 
  plot2dn(ptype,logflags,x,y,n1,n2,style,strflag,legend,brect,aaint,flagNax); 
}
...
void Objplot3d (fname,isfac,izcol,x,y,z,zcol,m,n,theta,alpha,legend,iflag,ebox,m1,n1,m2,n2,m3,n3,m3n,n3n)
     ...
     int *isfac,*n,*m,*iflag,*izcol;
     double *zcol;
     int * m1, *n1, *m2, *n2, *m3, *n3, *m3n, *n3n;/*Adding F.Leray 12.03.04 and 19.03.04*/
     char *fname,*legend; 
...
{  
  ...
  if ((sciGetGraphicMode (psubwin)->autoscaling)) {
    ...
    if (iflag[1] != 0) /* bounds_changed =*/ update_specification_bounds(psubwin, drect,3);
  } 
...
void Objfec (x,y,noeud,fun,n,m,strflag,legend,brect,aaint,Zminmax,Colminmax,ColOut,WithMesh,flagNax)
     ...
     int *n,*m,aaint[],Colminmax[],ColOut[];
     char legend[],strflag[];
     double *fun;
