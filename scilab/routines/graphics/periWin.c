/*------------------------------------------------------------------------
 *    Graphic library
 *    Copyright (C) 1998-2011 Enpc/Jean-Philippe Chancelier
 *    jpc@cermics.enpc.fr 
 *    Fabrice Leray 2004
 *    Allan CORNET 2004
 *--------------------------------------------------------------------------*/
...
static void SetWinhdc1(struct BCG *Xgc)
{
  if ( Xgc != (struct BCG *) 0 && Xgc->CWindow != (HWND) 0)
    {
...
int sci_exists_winid(int Num)
{
  WindowList *listptr = The_List;
  while ( listptr != (WindowList  *) 0 ) 
    {
      if ( listptr->winxgc.CurWindow == Num) 
	return TRUE;
      listptr =  (WindowList *)listptr->next;
    }
  return FALSE;
...
void C2F(setcurwin)(int *intnum,int * v2,int * v3,int * v4)
{ 
  struct BCG *bcgk=ScilabXgc ;
  integer ierr;
  /* send info to menu */
...
void C2F(setcolormap)(int *v1,int *v2,int *v3,int *v4,int *v5,int *v6,
		      double *a)
{
  unsigned long maxcol;
  int iPlanes = GetDeviceCaps(hdc,PLANES);
...
void C2F(setgccolormap)(int *v1,int *v2, double *a,struct BCG *XGC,int *v3)
{
  setcolormapg(XGC,v1,v2,a,v3);
}

void setcolormapg(struct BCG *Xgc,int *v1, int *v2, double *a, int *v3)
{
  int i,palstatus ,m;
  unsigned long maxcol;
  COLORREF  *c = (COLORREF *) NULL;
...
{
  *m = 0;
  if (  ScilabXgc == NULL) return 0;
  *m =  ScilabXgc->Numcolors;
  return ( ScilabXgc->CmapFlag  != 1) ? 1:0;
}
...
static void SetGraphicsVersion(struct BCG *XGC);
...
void C2F(MissileGCGetorSet)(char *str,integer flag,integer *verbose,integer *x1,
			    integer *x2,integer *x3,integer *x4,integer *x5,
			    integer *x6,double  *dv1)
{
	integer i ;
...
static struct BCG *AddNewWindow(WindowList **listptr)
{
  if ( *listptr == (WindowList *) NULL)
      ...
      else
	{
	  ...
	  /* Added by F.Leray set the current color to black (i.e.: 32+1=33)*/
	  (*listptr)->winxgc.CurColor = 32; 
...
{
  struct BCG *bcg = GetWindowXgcNumber(wincount);
  return ( bcg != NULL) ?  bcg->CWindow : NULL;
}
...
struct BCG *GetWinXgc(WindowList *listptr,int i)
{
  if (listptr == (WindowList  *) NULL) return NULL;
  if ((listptr->winxgc.CurWindow) == i) return &(listptr->winxgc);
...
void C2F(getwins)(int *Num,int *Ids,int *flag)
{
  WindowList *listptr = The_List;
  *Num = 0;
  if ( *flag == 0 )
...
void C2F(initgraphic)(char * string,int * v2,int * v3,int * v4, int *v5,int * v6,int * v7,
		      double * dv1, double * dv2, double * dv3, double * dv4)
{ 
  static char popupname[sizeof("ScilabGraphic")+32];
  static char winname[sizeof("BG")+32];
  static int EntryCounter = 0;
  struct BCG *NewXgc ;
  RECT rect,rect1;
  int WinNum;
  SCROLLINFO vertsi, horzsi;
  ...
  if (EntryCounter == 0)
    {
      ...
      if (! graphwin.hPrevInstance)
	{
	  CreateGraphClass();
	}
    }
  
  /* warning 26 janv 2011 
   * we create a new window and store its data 
   * in NewXgc. Then at the end ScilabXgc which is 
   * the current window should be equal to NewXgc.
   * But some of the functions below like ShowWindow 
   * can activate some events which can change ScilabXgc 
   * (more explicitely ScilabPaint on already existing windows).
   * It is thus very dangerous in this function to 
   * suppose that ScilabXgc remains equal to NewXgc. 
   */
  ...
  ScilabXgc= NewXgc;
  NewXgc->lpgw = &graphwin;
  sprintf(popupname,"ScilabGraphic%d", (int)WinNum);

  if (EntryCounter == 0)
    {
      /* ReadRegistryGraph takes care of graphwin.Origin and  graphwin.Size */
      ReadRegistryGraph(NewXgc);
      if ( NewXgc->lpgw->Size.x == CW_USEDEFAULT ) 
	NewXgc->lpgw->Size.x = 600;
      if ( NewXgc->lpgw->Size.y == CW_USEDEFAULT ) 
	NewXgc->lpgw->Size.y = 400;
    }
  
  NewXgc->Inside_init=1; /** to know that we are inside init code **/
  ...
  if (IsTKGraphicalMode())
      ...
      if (EntryCounter == 0)
	{
      ...
      /* set the graphics version using global versionflag variable */
      SetGraphicsVersion(NewXgc);
      StoreXgc(WinNum);
    ...
    {
      NewXgc->hWndParent = CreateWindow(szParentGraphClass, popupname,
					WS_OVERLAPPEDWINDOW,
      ...
      if (NewXgc->hWndParent == (HWND)NULL)
	{
      ...
      ShowWindow(NewXgc->hWndParent,  SW_SHOWNORMAL);
      NewXgc->Statusbar =  InitStatusBar (NewXgc->hWndParent);
      ShowWindow(NewXgc->Statusbar,  SW_SHOWNORMAL);
      GetWindowRect(NewXgc->Statusbar, &rect1) ;
      GetClientRect(NewXgc->hWndParent, &rect);
      MoveWindow(NewXgc->Statusbar, 0, rect.bottom -( rect1.bottom - rect1.top),
		 rect.right,  ( rect1.bottom - rect1.top), TRUE) ;

      sprintf((char *)winname,"BG%d", (int)WinNum);
      NewXgc->CWindowWidth =  rect.right;
      NewXgc->CWindowHeight = rect.bottom - ( rect1.bottom - rect1.top);

      NewXgc->CWindow = CreateWindow(szGraphClass, winname,
					WS_CHILD | WS_VSCROLL | WS_HSCROLL,
					0, graphwin.ButtonHeight,
					NewXgc->CWindowWidth,
					NewXgc->CWindowHeight,
					NewXgc->hWndParent,
					NULL, graphwin.hInstance,
					NewXgc);

      NewXgc->CurResizeStatus = 1;
      NewXgc->CWindowWidthView  = NewXgc->CWindowWidth;
      NewXgc->CWindowHeightView = NewXgc->CWindowHeight;

      /* definition des scroll bars verticalles */
      vertsi.cbSize = sizeof(SCROLLINFO);
      vertsi.fMask  = SIF_RANGE | SIF_PAGE | SIF_POS;
      vertsi.nMin   = 0;
      vertsi.nMax   = NewXgc->CWindowHeight;
      vertsi.nPage  = NewXgc->CWindowHeightView;
      vertsi.nPos   = 0;
      sciSetScrollInfo(NewXgc,SB_VERT, &(vertsi), TRUE);
      sciGetScrollInfo(NewXgc,SB_VERT, &vertsi);

      /* definition des scroll bars horizontalle */
      horzsi.cbSize = sizeof(SCROLLINFO);
      horzsi.fMask  = SIF_RANGE | SIF_PAGE | SIF_POS;
      horzsi.nMin   = 0;
      horzsi.nMax   = NewXgc->CWindowWidth;
      horzsi.nPage  = NewXgc->CWindowWidthView;
      horzsi.nPos   = 0;
      sciSetScrollInfo(NewXgc,SB_HORZ, &horzsi, TRUE);	  
      sciGetScrollInfo(NewXgc,SB_HORZ, &horzsi);

      if (NewXgc->CWindow == (HWND)NULL) 
	{
      ...
      ShowWindow(NewXgc->CWindow, SW_SHOWNORMAL);
      ShowWindow(NewXgc->hWndParent,  SW_SHOWNORMAL);
      graphwin.resized = FALSE;

      /*UpdateFileGraphNameMenu( NewXgc);
	LoadGraphMacros( NewXgc);*/
      ...
      SetMapMode(hdc, MM_TEXT);
      SetBkMode(hdc,TRANSPARENT);
      GetClientRect(NewXgc->CWindow, &rect);
      SetViewportExtEx(hdc, rect.right, rect.bottom,NULL);
      SetTextAlign(hdc, TA_LEFT|TA_BOTTOM);
      SetFocus( NewXgc->CWindow);
      SetActiveWindow( NewXgc->CWindow);
      ...
      if (EntryCounter == 0)
	{
      ...
      /* be sure that default is NewXgc */
      ScilabXgc = NewXgc;
      InitMissileXgc(PI0,PI0,PI0,PI0);/* a laisser ici */
      /* to be sure that current values are recorded */  
      /* set the graphics version using global versionflag variable */
      ...
      EntryCounter++;
      /* see ../wsci/wgmenu.c */
      CreateGedMenus(NewXgc);	  
      CreateGraphToolBar(NewXgc);
      ScilabXgc = NewXgc;
    }
  
  NewXgc->Inside_init=0;
}

static void CreateGraphClass(void)
{
...
{ 
  ...
  ScilabXgc->CurColorStatus = 1;
  /* This call initializes ScilabXgc->NumForeground = 32 
   * and ScilabXgc->NumBackground = 33
   */
  ...
  /* a faire aussi pour le n&b plus haut */
  C2F(setforeground)((i=ScilabXgc->NumForeground+1,&i),PI0,PI0,PI0);
  C2F(setbackground)((i=ScilabXgc->NumForeground+2,&i),PI0,PI0,PI0);
...
static void SetGraphicsVersion(struct BCG *xgc)
{
  xgc->graphicsversion = versionflag;
}
...
{ 
  ...
  old= SelectFont(hdc, (*FontTab)[i].hf[fsiz]);
}
  ...
  for (i =0 ; i < FONTMAXSIZE ; i++)
    {
      if ( (*FontTab)[1].hf[i] != NULL)
        {
	  HFONT old;
	  old=SelectFont(hdc, (*FontTab)[fid].hf[i]);
