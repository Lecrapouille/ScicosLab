...
void C2F(perigtk_analyze_points)(integer n, integer *vx, integer *vy, integer onemore);
...
static void xset_dashstyle (integer *v1,integer *v2,integer *v3);

/* declaration for xset('name',...) functions */

typedef void (xset_f) (integer * v1,integer * v2,integer * v3,integer * v4);
...
typedef void (xget_f) (integer *verbose, integer *x,integer *narg, double *dummy);
...
static int gtk_store_points (integer n, integer *vx,integer *vy,int  onemore);

typedef void (*r_c) (int x,int y,int w,int h);
...
/* to compile with gdk<2.4 */
  ...
  if ( click_event &&
    ...
    {
      if ( info.sci_click_activated == 1 )
	{
	  if ( info.getrelease == 0 ) return TRUE;
	  info.ok =1 ; info.win=  gc->CurWindow; info.x = event->x;  info.y = event->y;
...
static gint configure_event_plus(GtkWidget *widget, GdkEventConfigure *event, gpointer data)
{
  BCG *dd = (BCG *) data;
  ...
  if( (GTK_WIDGET_REALIZED(dd->drawing))) 
    {
      if ( info.sci_click_activated == 0 ) 
	{
	  PushClickQueue( dd->CurWindow,0,0,4000 ,0,0);
	}
      else 
	{
	  info.ok =1 ;  info.win=  dd->CurWindow; info.x = 0;  info.y = 0;
	  info.button = 4000;
	  gtk_main_quit();
	}
    }
...
static void xset_windowdim(integer *x, integer *y, integer *v3, integer *v4)
{
  ...
  if ( ScilabXgc->CurResizeStatus == 0) 
    ...
    {
      gint xx=0,yy=0;
      /* we compute the offset between the two windows dimensions */
      /*
	gdk_window_get_size (ScilabXgc->window->window,&xx,&yy);
	xx -= ScilabXgc->CWindowWidth;
	yy -= ScilabXgc->CWindowHeight;
      */
      /* we force the dimensions by changing the popup */
      gdk_window_resize(ScilabXgc->window->window,*x+xx ,*y+yy);
    }
}
...
{ 
  if ( *listptr == (WindowList *) NULL)
  ...
  else
    {
      ...
      /* 
	 WindowList *Lnext = (WindowList *) ((*listptr)->next);
	 return AddNewWindow( &Lnext) ;
      */
    }
}
...
static struct BCG *GetWinXgc (WindowList *,integer);
...
static int GTK_Open(struct BCG *dd, char *dsp, double w, double h)
{
  ...
  sprintf( gwin_name, "ScicosLab Graphic (%d)", dd->CurWindow );
  gtk_window_set_title (GTK_WINDOW (dd->window),  gwin_name);

  SciPath=getenv("SCI");
  if (SciPath!=NULL) {
    buf=g_build_filename(SciPath,ICON_NAME, NULL);
    gtk_window_set_icon(GTK_WINDOW(dd->window), create_pixbuf(buf));
    g_free(buf);
 }
  ...
  */
  ...
  gtk_signal_connect(GTK_OBJECT(dd->drawing), "configure_event",
		     (GtkSignalFunc) configure_event_plus, (gpointer) dd);
