...
static int event_select=5;/* record only press and release events, ignoring move. Note that configure is considered as a click*/
...
{
  static char buf[256];
  struct BCG *SciGc = GetWindowXgcNumber(win);
  if ( SciGc != NULL && strlen(SciGc->EventHandler)!=0) 
    {
...
int PushClickQueue(int win,int x,int y,int ibut, int motion,int release) 
{
  ...
  if (((event_select&2)&&motion)||((event_select&4)&&release)||(motion==0&&release==0))
      ...
      if (motion&&lastc>0 && (ClickBuf[lastc-1].motion && win== ClickBuf[lastc-1].win)) 
	{
      ...
      else if ( lastc > 0 && ibut == 4000 && ClickBuf[lastc-1].ibutton == ibut && win== ClickBuf[lastc-1].win )
	{
	  /* do not record multiple configure */
	}
      else {
