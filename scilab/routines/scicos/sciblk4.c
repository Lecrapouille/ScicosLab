...
int check_lst(void *lst1,void *lst2);
int check_lst_obj(int *il,int *ref_il);
 ...
 *
 * Alan   : Rewritten from original code of sciblk4.c
 *          of scicos 2.7.
 */
     ...
     integer flag;
{
  ...
  header = (int *) stk(*Lstk(Top_stack));

  /* check output list type and size */
  if (header[0]!=16) {
    ierr=-6;
    goto err;
  }
   ...
   ***********************/
   case 2 :
   {
    ...
    if (Blocks[0].nz != 0)
    {
     ...
     {
      ierr=check_lst_obj(il_z,(int *)Blocks[0].z);
      if (ierr!=0) goto err;
    ...
    if (Blocks[0].noz != 0)
    {
     ...
     if ((funtyp[kfun-1]==4) || (funtyp[kfun-1]==10004))
     {
      for(j=0;j<Blocks[0].noz;j++)
      {
       ...
       { 
        ne1=Blocks[0].ozsz[j];
        ierr=check_lst(il_oz,Blocks[0].ozptr[0]);
        if (ierr!=0) goto err;
  ...
  /* Restore initial position Top_stack */
  Top_stack=topsave;
  return;

 /* if error then restore initial position Top_stack
  * and set_block_error with flag -1 */
 err:
  Top_stack=topsave;
  if (ierr!=0) /*check_lst, var2sci or sci2var error*/
  {
   /* Please update me !*/
   if (ierr < 0) /*check_lst error*/
   {
    switch (ierr)
    {
     case -1  : Scierror(888,"check_lst : error %d. Bad discrete state type.\n",ierr);
                break;

     case -2  : Scierror(888,"check_lst : error %d. Bad discrete state size.\n",ierr);
                break;

     case -3  : Scierror(888,"check_lst : error %d. A type in discrete state have changed.\n",ierr);
                break;

     case -4  : Scierror(888,"check_lst_obj : error %d. Size in discrete state have changed.\n",ierr);
                break;

     case -5  : Scierror(888,"check_lst_obj : error %d. A subtype in discrete state have changed.\n",ierr);
                break;
                
     case -6  : Scierror(888,"error %d. The output block list doesn''t have the good type.\n",ierr);
                break;

     case -7  : Scierror(888,"error %d. The output block list doesn''t have the good size.\n",ierr);
                break;

     default : Scierror(888,"check_lst : error %d. Undefined error.\n",ierr);
               break;
    }
   }
   else if (ierr < 1000) /*var2sci error*/
   {
    switch (ierr)
    {
...
/* check_lst function to verify if two
 * lists have same objects with same size
 *
 * Input parameters :
 * *lst1   : void ptr of a ScicosLab list to compare (comes from ScicosLab)
 * *lst2   : void ptr of a ScicosLab list to compare (ref comes from scicos.c).
 *
 * Output parameters : int (<0), error flag
 *                     (0 if no error)
 *
 * 30/08/09, Alan   : Initial version.
 *
 */
...
int check_lst(void *lst1,void *lst2)
{
 ...
 /* check lst_obj */
 for (i=0;i<head[1];i++) {
   ref_il = (int *) listentry(ref_head,i+1);
   il     = (int *) listentry(head,i+1);
   ierr   = check_lst_obj(il,ref_il);
   if (ierr!=0) {
     return ierr;
   }
 }
...
/* check_lst_obj function to verify if two
 * objects have same objects with same size
 *
 * Input parameters :
 * *il     : void ptr of a ScicosLab object to compare (comes from ScicosLab)
 * *il_ref : void ptr of a ScicosLab object to compare (ref comes from scicos.c).
 *
 * Output parameters : int (<0), error flag
 *                     (0 if no error)
 *
...
int check_lst_obj(int *il,int *ref_il)
{
 ...
 switch(typ)
 {
  ...
  case 15 :
  case 16 :
  case 17 : /* check numbers of object */
            if (il[1]!=ref_il[1]) {
             return -4;
            }
            /* check lst_obj */
            for (i=0;i<ref_il[1];i++) {
              ierr   = check_lst_obj((int *) listentry(il,i+1),(int *) listentry(ref_il,i+1));
              if (ierr!=0) {
               return ierr;
              }
            }
