...
#define freeallx \
	      ...
	      if (( ng>0 )&& (*neq>0)) FREE(ida_data->gwork);\
...
int C2F(grblk)(int *, double *, double *, integer *, double *);
...
int KinJacobians0(long int , DenseMat , N_Vector , N_Vector , void *, N_Vector , N_Vector );
int KinJacobians1(long int , DenseMat , N_Vector , N_Vector , void *, N_Vector , N_Vector );
...
IMPORT struct {int cosd;} C2F(cosdebug);
IMPORT struct {int counter;} C2F(cosdebugcounter);
...
static integer nblk, nordptr, nlnk, nx, ng, ncord, noord, nzord;
static integer niord,nordclk,niord,nmod;
...
extern struct {integer iero;} C2F(ierode);
extern struct {integer kfun;} C2F(curblk);
extern struct {integer halt;} C2F(coshlt);
extern struct {char buf[4096];} coserr;
extern struct {integer callerid;} C2F(lscallerid);

struct {double scale;} C2F(rtfactor);
struct {int solver;  } C2F(cmsolver);
...
static integer *clkptr, *ordptr, *ordclk, *cord, *iord, *oord,  *zord,  *critev,  *zcptr;
                ...
                double *simpar,integer *flag__,integer *ierr_out)
{
  ...
  *ierr = 0;
  ...
  if(nx>0) { /* xprop */
    if((xprop=(int*) MALLOC(sizeof(int)*nx+sizeof(double)*nx*2))== NULL ) {
      *ierr =5;
      return 0;
    }
    alpha=(double*) (xprop+nx);
    beta =(double*) (alpha+nx);    
  }
  ...
  for (kf = 0; kf < nblk; ++kf) {
    ...
    if (Blocks[kf].nx!=0) {
      ...
      Blocks[kf].alpha=&(alpha[xptr[kf+1]-1]);
      Blocks[kf].beta =&( beta[xptr[kf+1]-1]);      
    }
  ...
  if (*flag__ == 1) { /*start*/
    ...
    /*     integration */
    phase=1;    
    if (C2F(cmsolver).solver == 0) {      /*  Lsodar: Method: BDF,   Nonlinear solver= NEWTON     */
      cossim(t0);
      ...
      cossim(t0);
    }else if (C2F(cmsolver).solver == 5) {/*  CVODE: Method: ADAMS, Nonlinear solver= FUNCTIONAL */
      cossim(t0);
...
int Setup_Cvode(void **cvode_mem,int solver,int N, N_Vector *y,User_CV_data *cv_data,\
  ...
  *cvode_mem = NULL;
  ...
  switch (solver)
    {
    case 1:   *cvode_mem = CVodeCreate(CV_BDF, CV_NEWTON);break;
    case 2:   *cvode_mem = CVodeCreate(CV_BDF, CV_FUNCTIONAL);break;
    case 3:   *cvode_mem = CVodeCreate(CV_ADAMS, CV_NEWTON);break;
    case 4:   *cvode_mem = CVodeCreate(CV_ADAMS, CV_FUNCTIONAL);break;
    }    
...
int Setup_Lsodar(int *nrwp, int *niwp, double **rhot, int **ihot, double hmax, int *iopt, int N, int ng1) 
{
  int jj;
  int c__1 = 1;
  int c__0 = 0;
  ...
  *niwp = N + 20 + ng1;/* + ng is for change in lsodar2 to handle masking */
  ...
  jj=*niwp+1; C2F(iset)(&jj, &c__0, *ihot, &c__1); /*set to 0  */
  jj=*nrwp+1; C2F(dset)(&jj, &c_b14,*rhot, &c__1); /*set to 0.0*/
...
void cossim(double *told)
{
  ...
  int cnt=0;
  double *rhot=NULL;
  integer *ihot=NULL,niwp,nrwp;
  integer itask;
  integer jt,kk,lyh;
  integer istate,iopt, itt;
  ...
  zcros=NULL;
  if (ng>0) {
    if((jroot=MALLOC(sizeof(int)*ng*2))== NULL ){
      *ierr =10000;
      return;
    }
    for ( jj = 0 ; jj < ng*2 ; jj++ )
  ...
  hmax_auto=(hmax>0) ? hmax: (*tf/100.0);
  if ( *(scs_imp->noord) ==0)   hmax_auto=deltat;

  switch (C2F(cmsolver).solver) { 
  case 0:   /* LSODAR initialization*/
    flag=Setup_Lsodar(&nrwp,&niwp, &rhot, &ihot, hmax_auto, &iopt,*neq, ng);
    break;
  case 1:
  case 2:
  case 3:
  case 4: /* CVODE does not work with NEQ==0 */
    flag=Setup_Cvode(&cvode_mem,C2F(cmsolver).solver,*neq,&y,&cv_data,reltol,&abstol,*told,x, ng, hmax_auto);
    break;
  case 5: /* DOPRI5 initialization*/
    flag=Setup_dopri5(&dopri5_mem, *neq, DP5simblk, *told,  *tf, reltol, &abstol, 0, hmax_auto, ng, DP5grblk,
		      &dopri5_udata);
    break;
  } 
  ...
  *ierr = 0;
  ...
  if (ng>0){ /* storing ZC signs just after a solver call*/
    ...
    for (jj = 0; jj < ng; ++jj)
      if(g[jj]>=0) jroottmp[jj]=5;else jroottmp[jj]=-5;
  }
  /*--discrete zero crossings----dzero--------------------*/
  ...
  while(*told < *tf) {
	...
	tnext =  min(*told + hmax_auto,*told+deltat);
	...
	L30:
	  if (rhotmp > *tf + ttol) rhotmp = *tf + ttol;
	  if (rhotmp < tstop-ttol) {
	    hot = 0;
	  }
	}
	tstop = rhotmp;
	...
	case 4:
	  if (hot==0){ /* hot==0 : cold restart*/
	    ...
	    if (check_flag(&flag, "CVodeSetStopTime", 1)) {*ierr=300+(-flag);freeall;return; }	    
	    flag =CVodeReInit(cvode_mem, CVsimblk, (realtype)(*told), y, CV_SS, reltol, &abstol);
	    if (check_flag(&flag, "CVodeReInit", 1)) {*ierr=300+(-flag);freeall; return; }
	  }
	  break;
	...
	if (ng>0 && hot==0){
	  ...
	  if (*ierr != 0) {freeall;return;}
	  for (jj = 0; jj < ng; ++jj) {
	    if((g[jj]>=0.0)&&(jroottmp[jj]==-5)) {Discrete_Jump=1;jroottmp[jj]=1;}
	    else if((g[jj]<0.0)&&(jroottmp[jj]==5)) {Discrete_Jump=1;jroottmp[jj]=-1;}
	    else jroottmp[jj]=0;
	  }
	}
	/*--discrete zero crossings----dzero--------------------*/
	...
	if (Discrete_Jump==0){/* if there was a dzero, its event should be activated*/
	  ...
	  if(hot==0) {tn=*told; X_contain_xn=1;zcrossing_unhandeled=0;}
	  flag=0;
	  if (!zcrossing_unhandeled){
		...
		switch (C2F(cmsolver).solver )
		  ...
		  case 0: 		
		    lyh=21+3*ng;
		    kk=0;
		    C2F(intdy) (&tn, &kk,  &(rhot[lyh]), neq,  x, &itt); 
		    break;
		  case 1:
		  case 2:
		  case 3:
		...
		{ 
		case 0: 
		  C2F(lsodar2)(C2F(simblk), neq, x, told, &t, &c__1, &rtol,&Atol, &itask, &istate, &iopt,
			       &rhot[1], &nrwp, &ihot[1], &niwp, &jdum, &jt, C2F(grblk), &ng, jroot);
		  tn=rhot[13];		
		  if (istate>0) flag=istate+200;else flag=istate-200;
		...
		case 4: 
		  ...
		  break;
		case 5: 
		  flag=dopri5_solve (dopri5_mem, told,  t, x, hot);
		  break;
		}
	      if ((flag==LSODAR_ZERO_DETACH_RETURN)||(flag==LSODAR_ROOT_RETURN)||
	  ...
	  if (*told<=tn) {
		...
		lyh=21+3*ng;
		kk=0;
		C2F(intdy) (told, &kk,  &(rhot[lyh]), neq,  x, &istate); 
		break;
	      case 1:
	      case 2:
	      case 3:
	...
	if (flag>=0){
	  ...
	  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))
	    sciprint("****Solver: cannot converge at time=%g (stiff region, change RTOL and ATOL)\r\n",*told);	  
	  hot = 0;
          cnt++;
          if (cnt>5) {
	    *ierr=300+(-flag);
	  ...
	  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))
	    sciprint("****Solver: too much work at time=%g (stiff region, change RTOL and ATOL)\r\n",*told);	  
	  hot=1;
	  istate=2;
	  cnt++; if (cnt>5) {hot=0;};
	}else{
	...
	if (flag == CV_ZERO_DETACH_RETURN || flag == LSODAR_ZERO_DETACH_RETURN||
	    flag == DP5_ZERO_DETACH_RETURN ){hot=0;  zcrossing_unhandeled=0;};  /* new feature of sundials, detects zero-detaching */
	  ...
	  for (jj = 0; jj < ng; ++jj) {
		...
		if (Blocks[C2F(curblk).kfun-1].nevout > 0) {
		  ...
		  for (j=0;j<Blocks[C2F(curblk).kfun-1].nevout;j++) {
		    Blocks[C2F(curblk).kfun-1].evout[j]=-1;
		  }
		  ...
		  for (k = 0; k < Blocks[C2F(curblk).kfun-1].nevout; ++k) {
		    ...
		    if (isinf(Blocks[C2F(curblk).kfun-1].evout[k])) {
		...
		/*     .              update state */
                if ((Blocks[C2F(curblk).kfun-1].nx > 0) || (Blocks[C2F(curblk).kfun-1].nz > 0) ||(Blocks[C2F(curblk).kfun-1].noz > 0) ||(*Blocks[C2F(curblk).kfun-1].work != NULL)) {
...
//double reltol, double *abstol,double t0,double *x,int ng1, double hmax){
int Setup_IDA(void **ida_mem, int N, N_Vector *yy, double *x, N_Vector *yp, double *xd, N_Vector *IDx,
...
void cossimdaskr(double *told)
{
  ...
  static integer j, k;
  static double t;
  static integer kk,jj, jt;
  static integer  ntimer;
  static double rhotmp,tstop, hmax_auto;
  static integer inxsci;
  static integer kpo, kev;

  integer *jroot=NULL,*zcros=NULL;
  integer maxord;
  integer *Mode_save;
  integer Mode_change=0;
  ...
  if (ng!=0) {
    ...
    jroottmp=jroot+ng;
    for ( jj = 0 ; jj < ng*2 ; jj++ )  jroot[jj] = 0 ;
    if((zcros=MALLOC(sizeof(int)*ng))== NULL ){
      *ierr =10000;
      if (ng!=0) FREE(jroot);
      return;
  ...
  if (*neq>0) {
    flag=Setup_IDA( &ida_mem, *neq, &yy, x, &yp,xd, &IDx,reltol,&abstol,*told, ng,hmax_auto, &ida_data);
    if (flag<0) {
      *ierr =10000;
      if (*neq>0 ) FREE(tmpneq);
      if (nmod) FREE(Mode_save);
  ...
  *ierr = 0;
  ...
  while (*told < *tf) {
	...
	if (hot == 0){ /* CIC calculation when hot==0 */
	  ...
	  Jacobians(*neq, (realtype) (*told), yy, yp,	bidon, (realtype) CJ, ida_data, TJacque, tempv1, tempv2, tempv3);
	  ...
	  for(j=0;j<=N_iters;j++){/* counter to reevaluate the
	    ...
	    if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))
	      {
		if (flagr>=0) {
		  sciprint("**** Solver succesfully initialized *****\r\n" );
		}
		else{
		  sciprint("**** Solver failed to initialize ->try again *****\r\n" );
		}
	      }
	    /*-------------------------------------*/
	...
	if (flagr == IDA_ZERO_DETACH_RETURN){hot=0; zcrossing_unhandeled=0;}; /* new feature of sundials, detects unmasking */
...
/* callf */
/*void callf(double *t,double *xtd,double *xt,double *residual,double *g,integer *flag)*/
void callf(double *t, scicos_block *block, int *flag)
{
  double* args[SZ_SIZE];
  integer sz[SZ_SIZE];
...
{
  int nantest=0,i;
  C2F(dset)(neq1, &c_b14,xcdot , &c__1);
  C2F(ierode).iero = 0;
...
int CVgrblk(realtype t, N_Vector yy, realtype *gout, void *g_data)
{
  ...
  C2F(grblk)(neq, &tx, x, &ng, (double*)gout);
...
int simblkdaskr(realtype tres, N_Vector yy, N_Vector yp, N_Vector resval, void *rdata)
{
  ...
  int qlast;
  int jj,flag, nantest;
  /*  int phase= GetSimulationPhase(Blocks); */
...
/* Subroutine rmevs  */
void rmevs(integer *evtnb, integer *pointi)
{
  static integer i;
  if (evtspt[*evtnb] != -1) {
    /*    if (TCritWarning==0){
      sciprint("\n\r Warning: a programmed output event at t=%g is deprogrammed by a block",tevts[*evtnb]);
      sciprint("\n\r Please check your model for possible error. This warning is reported once.\n\r");
      TCritWarning=1; 
    }*/
    if ((evtspt[*evtnb] == 0) && (*pointi ==*evtnb)) {
      evtspt[*evtnb] = -1;
      *pointi=0;
    }else if (*pointi == *evtnb) {
      *pointi =evtspt[*evtnb]; 
      evtspt[*evtnb]=-1;
    }else {
      i= *pointi;
...
/* Subroutine addevs */
void addevs(double t, integer *evtnb, integer *ierr1)
{
  static integer i, j;
  ...
  *ierr1 = 0;
  if (evtspt[*evtnb] != -1) {
    if (TCritWarning==0){
      ...
      sciprint("\n\r    (already programmed) event at  t=%g. There may be an error in",tevts[*evtnb]);
...
/* Subroutine putevs */
void putevs(double *t, integer *evtnb, integer *ierr1)
{
  /* Function Body */
  *ierr1 = 0;
  if (evtspt[*evtnb] != -1) {
...
{ /* initialisation (propagation of constant blocks outputs) */
  ...
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  idoit(): %f\r\n",*told);
...
{ /* propagation of blocks outputs on discrete activations */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  doit(): %f\r\n",*told);
...
{ /* propagation of continuous blocks outputs */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  cdoit(): %f\r\n",*told);
...
{ /* update states & event out on discrete activations */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  ddoit(): %f\r\n",*told);
  ...
  for (i=0;i<kiwa;i++) {
	...
	scs_imp->blocks[*kf-1].xd = &scs_imp->xd[scs_imp->xptr[*kf-1]-1];
      ...
      if (scs_imp->blocks[*kf-1].nevout > 0) {
	    ...
	    if (isinf(scs_imp->blocks[*kf-1].evout[j])) {
...
/* Subroutine edoit */
void edoit(double *told, integer *kiwa)
{ /* update blocks output on discrete activations */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  edoit(): %f\r\n",*told);
...
{ /* update blocks derivative of continuous time block */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  odoit(): %f\r\n",*told);
  ...
  for (ii = 0; ii < *(scs_imp->noord); ii++) {
    kf = &scs_imp->oord[ii];
    C2F(curblk).kfun = *kf;
    if (scs_imp->blocks[*kf-1].nx > 0 || *scs_imp->blocks[*kf-1].work !=NULL) {
...
{ /* update blocks xproperties of continuous time block */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  reinitdoit(): %f\r\n",*told);
...
/* Subroutine ozdoit */
void ozdoit(double *told, double *xt, double *xtd, integer *kiwa)
{ /* update blocks output of continuous time block on discrete activations */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  ozdoit(): %f\r\n",*told);
...
{ /* update blocks zcross of continuous time block  */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  zdoit(): %f\r\n",*told);
...
{ /* update blocks jacobian of continuous time block  */
  ...
  scs_imp = getscicosimportptr();
  if ((C2F(cosdebug).cosd >= 1) && (C2F(cosdebug).cosd != 3))  sciprint("  Jdoit(): %f\r\n",*told);
...
{ /* synchro blocks computation  */
  ...
  /* eselect blk */
    ...
    switch(outtbtyp[-1+inplnk[inpptr[kf-1]-1]])
      {
      case SCSREAL_N    : outtbdptr=(SCSREAL_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtbdptr,scs_imp->blocks[kf-1].nevout),1);
	break;
	...
	break;
      ...
      case SCSINT16_N   : outtbsptr=(SCSINT16_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtbsptr,scs_imp->blocks[kf-1].nevout),1);
	break;

      case SCSINT32_N   : outtblptr=(SCSINT32_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtblptr,scs_imp->blocks[kf-1].nevout),1);
	break;

      case SCSUINT8_N   : outtbucptr=(SCSUINT8_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtbucptr,scs_imp->blocks[kf-1].nevout),1);
	break;

      case SCSUINT16_N  : outtbusptr=(SCSUINT16_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtbusptr,scs_imp->blocks[kf-1].nevout),1);
	break;

      case SCSUINT32_N  : outtbulptr=(SCSUINT32_COP *)outtbptr[-1+inplnk[inpptr[kf-1]-1]];
	i=max(min((integer) *outtbulptr,scs_imp->blocks[kf-1].nevout),1);
	break;
...
{ /* synchro blocks with zcross computation  */
  ...
  /* eselect blk */
	...
	i=max(min((integer) *outtbcptr,scs_imp->blocks[kf-1].nevout),1);
	break;
	      ...
	      N_Vector tempv1, N_Vector tempv2, N_Vector tempv3)
{
  ...
  *ierr= 0;
  ...
  /*----------------------------------------------*/
  for (j=0;j<nx*nx+ nx*ni+ no*nx+ no*ni;j++)     Fx[j]=0.0;/* Filling up FX:Fu:Gx:Gu*/  
  job=1;/* read jacobian through flag=10; */
  Jdoit(&ttx, xc, xcdot, &Fx[-m], &job);/* Filling up the FX:Fu:Gx:Gu*/
  if (*ierr!=0) { sciprint("\n\r error in Jacobian");return -1;}
  /*-------------------------------------------------*/
...
int read_xml_initial_states(int nvar,const char * xmlfile, char **ids, double *svars){
  ...
  for (i=0;i<nvar;i++){
    ...
    else   sciprint("Cannot find: %s in the XML file \n",ids[i]);
  }
  ezxml_free(model);
  return 0;
}
...
int Convert_number (char *s, double *out)
{
  ...
  if (s != endp && *endp == '\0'){
    ...
    *out=d;
    ...
    l = strtol(s, &endp, 0);
    if (s != endp && *endp == '\0'){
      /*printf("  It's an integer with value %ld ", 1); */
      *out=(double)l;
      return 0;
    }else{
      /*printf("  string "); */
...
int CallKinsol(double *told)
{
  ...
  kin_mem = KINCreate(); if(kin_mem == NULL) {FREE(Mode_save);N_VDestroy_Serial(y);N_VDestroy_Serial(yscale);N_VDestroy_Serial(fscale);return -1;}

  if ((kin_data = (User_KIN_data) MALLOC(sizeof(User_KIN_data)))==NULL){
    FREE(Mode_save);N_VDestroy_Serial(y);N_VDestroy_Serial(yscale);N_VDestroy_Serial(fscale);KINFree(&kin_mem);return -1;
  }
  ...
  if ((kin_data->rwork = (double *) MALLOC(Jactaille * sizeof(double)))==NULL){
    FREE(Mode_save);N_VDestroy_Serial(y);N_VDestroy_Serial(yscale);N_VDestroy_Serial(fscale);KINFree(&kin_mem);FREE(kin_data);return -1;
  }
  ...
  for(k=0;k<=N_iters;k++){ /* loop for mode fixin*/
	...
	fsdata[j]=(5*fsdata[j]+1/abs(fi))/6;	
      ...
      if (status>=0) break;
      C2F(sxevents)(); if(C2F(coshlt).halt != 0){C2F(coshlt).halt = 0;freekinsol;return 0;}
    }
    /*---------end of KINSOL calls-----------*/
    if (phase==2 ) {
      for (j = 0; j < nmod; ++j) {
	...
	if(Mode_save[j] != mod[j]) {Mode_change=1; break; }
      ...
      if (status<0 && k>=N_iters-0) { /*Retrying with phase=1 */
	phase=1;	      
      }
    } else {
...
/*------------------------------------------=============+++++++++++===========---------------------*/
int KinJacobians0(long int n, DenseMat J, N_Vector u, N_Vector fu, void *jac_data, N_Vector tmp1, N_Vector tmp2)
{
...
int KinJacobians1(long int Neq, DenseMat Jacque, N_Vector yy, N_Vector resvec, void *jac_data, N_Vector tmp1, N_Vector tmp2)
{
  double  ttx;
  ...
  *ierr= 0;
  ...
  for(i=0;i<m;i++){
    xi = xc[i];
    ujscale = ONE/uscale_data[i];
    sign = (xi >= ZERO) ? ONE : -ONE;
