 ...
 * int intcoserror(fname,fname_len)
...
int inttimescicos(fname,fname_len)
     /* renvoi le temps de simulation t=GetScicosTime(block) */
     char *fname;
     unsigned long fname_len;
{
  int i,j,l1;
 ...
 *
 * A.Layec : Rewritten from original fortran
...
C2F(scicos)(l_state_x,l_sim_xptr,l_state_z,
 ...
 if (ierr>0)
 {
  switch (ierr)
  {
   ...
   case 24  : strcpy(C2F(cha1).buf,"You have changed a parameter in your model, but the model has been "
   ...
   case 26  : strcpy(C2F(cha1).buf,"The number of parameter provided by Scicos blocks is different from what "
                                   "expected by the code generated by the Modelica compiler. "
                                   "You might have relaxed a parameter using FIXED property (i.e., fixed=false) in a Modelica model. "
   ...
   case 202  : strcpy(C2F(cha1).buf,"IDA_ILL_INPUT: One of the input arguments was illegal. "
                                    "This errors message may be returned if the linear solver function initialization "
                                    "(called by the user after calling IDACreate) failed to set the linear solver-specific lsolve field in ida_mem.");
               C2F(curblk).kfun=0;
               break;
   ...
   case 204  : strcpy(C2F(cha1).buf,"IDA_TOO_MUCH_WORK: The solver took mxstep internal steps but could not reach tout. "
                                    "The default value for mxstep is MXSTEP_DEFAULT = 500.");
               C2F(curblk).kfun=0;
               break;

   case 205  : strcpy(C2F(cha1).buf,"IDA_TOO_MUCH_ACC: The solver could not satisfy the accuracy "
                                    "demanded by the user for some internal step.");
               C2F(curblk).kfun=0;
               break;

   case 206  : strcpy(C2F(cha1).buf,"IDA_ERR_FAIL: Error test failures occurred too many times "
                                    "(=MXETF = 10) during one internal step or occurred with |h|=h_min");
               C2F(curblk).kfun=0;
               break;

   case 207  : strcpy(C2F(cha1).buf,"IDA_CONV_FAIL: IDACalcIC->Failed to get convergence of the Newton iterations. "
                                    "Or IDA_solve->Convergence test failures occurred too many times (= MXNCF = 10) during one internal step.");
               C2F(curblk).kfun=0;
               break;
   ...
   case 213  : strcpy(C2F(cha1).buf,"IDA_REP_RES_ERR: The user''s residual function repeatedly returned a "
   ...
   case 216  : strcpy(C2F(cha1).buf,"IDA_BAD_EWT: Some component of the error weight vector is zero (illegal), "
                                    "either for the input value of y0 or a corrected value.");
               C2F(curblk).kfun=0;
               break;

   case 217  : strcpy(C2F(cha1).buf,"IDA_FIRST_RES_FAIL: The user''s residual routine returned  a recoverable "
                                    "error flag on the first call, but IDACalcIC was unable to recover.");
               C2F(curblk).kfun=0;
               break;

   case 218  : strcpy(C2F(cha1).buf,"IDA_LINESEARCH_FAIL: The Linesearch algorithm failed to find a solution "
                                    "with a step larger than steptol in weighted RMS norm.");
               C2F(curblk).kfun=0;
               break;

   case 219  : strcpy(C2F(cha1).buf,"IDA_NO_RECOVERY: The user''s residual routine, or the linear solver''s "
   ...
   case 237  : strcpy(C2F(cha1).buf,"IDACalc cannot find the initial condition at this point. "
   ...
   case 301  : strcpy(C2F(cha1).buf,"CV_TOO_MUCH_WORK: The solver took mxstep internal steps but could not reach ''tout''. "
                                    "The default value for mxstep is MXSTEP_DEFAULT = 500.");
               C2F(curblk).kfun=0;
               break;
   ...
   case 303  : strcpy(C2F(cha1).buf,"CV_ERR_FAILURE: Error test failures occurred too many times "
                                    "(=MXETF = 7) during one internal step or occurred with |h|=h_min ");
               C2F(curblk).kfun=0;
               break;

   case 304  : strcpy(C2F(cha1).buf,"CV_CONV_FAILURE: Convergence test failures occurred too many times "
   ...
   case 309  : strcpy(C2F(cha1).buf,"CV_FIRST_RHSFUNC_ERR: The right-hand side function (user supplied ODE) "
                                    "had a recoverable error at th efirst call");
               C2F(curblk).kfun=0;
               break;

   case 310  : strcpy(C2F(cha1).buf,"CV_REPTD_RHSFUNC_ERR: Convergence tests occurred too many times due to repeated "
   ...
   case 311  : strcpy(C2F(cha1).buf,"CV_UNREC_RHSFUNC_ERR: The right-hand side function (user supplied ODE) had "
   ...
   case 322  : strcpy(C2F(cha1).buf,"CV_ILL_INPUT: indicating an input argument was illegal. "
   ...
   case 323  : strcpy(C2F(cha1).buf,"CV_NO_MALLOC: indicating that cvode_mem has not been allocated "
                                    "(i.e., CVodeMalloc has not been called).");
               C2F(curblk).kfun=0;
               break;

   case 324  : strcpy(C2F(cha1).buf,"CV_BAD_K: k (the order of the derivative of y to be computed) "
   ...
   case 403  : strcpy(C2F(cha1).buf,"KIN_NO_MALLOC: Additional system memory has not yet been  allocated for vector storage "
                                    "(forgot to call the  KINMalloc routine).");
               C2F(curblk).kfun=0;
               break;

   case 404  : strcpy(C2F(cha1).buf,"KIN_MEM_FAIL: An error occurred during memory allocation "
                                    "either insufficient system resources are available "
                                    "or the vector kernel has not yet been initialized.");
               C2F(curblk).kfun=0;
               break;

   case 405  : strcpy(C2F(cha1).buf,"KIN_LINESEARCH_NONCONV:  The line-search algorithm was unable to find an iterate sufficiently distinct "
                                    "from the current iterate failure to satisfy the sufficient decrease condition could mean the current iterate "
                                    "is close to an approximate solution of the given nonlinear system, the finite-difference  approximation "
                                    "of the matrix-vector product  J(u)*v is inaccurate, or the real scalar scsteptol is too large.");
               C2F(curblk).kfun=0;
               break;
   ...
   case 407  : strcpy(C2F(cha1).buf,"KIN_MXNEWT_5X_EXCEEDE: Five consecutive steps have been taken that satisfy the following inequality: "
                                    "||uscale*p||_L2 > 0.99*mxnewtstep where p denotes the current step and mxnewtstep is a real scalar "
                                    "upper bound on the scaled step length such a failure may mean ||fscale*func(u)||_L2 asymptotes "
                                    "from above to a finite value, or the real scalar mxnewtstep is too small.");
               C2F(curblk).kfun=0;
               break;

   case 408 : strcpy(C2F(cha1).buf,"KIN_LINESEARCH_BCFAI: The line search algorithm (implemented in KINLineSearch) was unable "
                                   "to satisfy the beta-condition for MXNBCF + 1 nonlinear iterations (not necessarily consecutive), "
                                   "which may indicate the algorithm is making  poor progress.");
               C2F(curblk).kfun=0;
               break;

   case 409  : strcpy(C2F(cha1).buf,"KIN_LINSOLV_NO_RECOVERY: The user-supplied routine psolve encountered a recoverable error, "
   ...
   case 411  : strcpy(C2F(cha1).buf,"KIN_LSETUP_FAIL: The user-supplied routine pset (used to compute the preconditioner) "
                                    "encountered an unrecoverable error.");
               C2F(curblk).kfun=0;
               break;

   case 412  : strcpy(C2F(cha1).buf,"KIN_LSOLVE_FAIL: Either the user-supplied routine psolve (used to to solve the "
  ...
  case 414   : strcpy(C2F(cha1).buf,"KIN_FIRST_SYSFUNC_ERR: The system function failed at the first call. "
  ...
  case 415   : strcpy(C2F(cha1).buf,"KIN_REPTD_SYSFUNC_ERR: The system function failed repeatedly. "
                                    "Please verify your model. There might be an error in the computing function, "
                                    "for example a function may be called with illegal inputs");
               C2F(curblk).kfun=0;
               break;

  case 416  :  strcpy(C2F(cha1).buf,"KIN_NAN_ERR: The residual function returns NAN. "
                                    "Please verify your model: some functions might be called with illegal inputs.");
               C2F(curblk).kfun=0;
               break;

  case 501  :  strcpy(C2F(cha1).buf,"LSODAR_TOO_MUCH_WORK: an excessive amount of work (more than mxstep steps) "
                                    "was done on this call, before completing the requested task, but the integration was otherwise successful as far as t. "
                                    "(mxstep is an optional input and is normally 500.) to continue, the user may simply reset istate to a value .gt. 1 "
  ...
  case 502  :  strcpy(C2F(cha1).buf,"LSODAR_TOO_MUCH_ACC: too much accuracy was requested for the precision of the machine being used. "
                                    "This was detected before completing the requested task, but the integration was successful as far as t. "
                                    ...
                                    "(note.. if this condition is detected before taking any steps, then an illegal input return (istate = -3) occurs instead.)");
               C2F(curblk).kfun=0;
               break;

  case 503  :  strcpy(C2F(cha1).buf,"LSODAR_ILL_INPUT: illegal input was detected, before taking any integration steps. "
                                    "See written message for details. "
                                    "Note..  if the solver detects an infinite loop of calls to the solver with illegal input, it will cause the run to stop.");
               C2F(curblk).kfun=0;
               break;

  case 504  :  strcpy(C2F(cha1).buf,"LSODAR_ERR_FAILURE: there were repeated error test failures on one attempted step, before completing the requested  task, "
                                    "but the integration was successful as far as t. "
                                    "The problem may have a singularity, or the input  may be inappropriate.");
               C2F(curblk).kfun=0;
               break;

  case 505  :  strcpy(C2F(cha1).buf,"LSODAR_CONV_FAILURE: there were repeated convergence test failures on one attempted step, before completing the requested task, "
                                    "but the integration was successful as far as t. "
                                    "This may be caused by an inaccurate jacobian matrix, if one is being used.");
               C2F(curblk).kfun=0;
               break;

  case 506  :  strcpy(C2F(cha1).buf,"LSODAR_BAD_EWT: ewt(i) became zero for some i during the integration. "
                                    "Pure relative error control (atol(i)=0.0) was requested on a variable which has now vanished. "
                                    "The integration was successful as far as t.");
               C2F(curblk).kfun=0;
               break;

  case 507  :  strcpy(C2F(cha1).buf,"LSODAR_I_R_HOT_SHORT: the length of rwork and/or iwork was too small to proceed, but the integration was successful as far as t. "
                                    "This happens when lsodar chooses to switch methods but lrw and/or liw is too small for the new method.");
               C2F(curblk).kfun=0;
               break;

  case 551  :  strcpy(C2F(cha1).buf,"DOPRI5: The Dopri5 solver cannot converge. Two much steps have been taken.");
               C2F(curblk).kfun=0;
               break; /* DP5_TOO_MUCH_WORK defined in dopri5m.h*/

  case 555  :  strcpy(C2F(cha1).buf,"DOPRI5: the step size becomes too small.");
               C2F(curblk).kfun=0;
 ...
 *                 at the top+1 position of the stack
 *
 ...
 *
 * Alan : Rewritten from original fortran
                                                ...
                                                )
   {
    ...
    if (ierr==TRUE_)
    {
     ...
     {/* for each block, call createblklist */
      ...
      /* call createblklist */
      ierr=createblklist(&ptr_scsblk[k], &errc,i,funtyp[k]);
 ...
 unsigned long fname_len;
{
 ...
 for(i=0;i<n_lnktyp;i++)
 {
  ...
  switch (lnktyp[i])
  {
   ...
   case 3  : if ((ptr_i=MALLOC(nm*sizeof(SCSINT32_COP)))==NULL)
             {
              ierr=-1;
              break;
             }
   ...
   case 4  : if ((ptr_s=MALLOC(nm*sizeof(SCSINT16_COP)))==NULL)
             {
              ierr=-1;
              break;
             }
   ...
   case 6  : if ((ptr_ui=MALLOC(nm*sizeof(SCSUINT32_COP)))==NULL)
             {
              ierr=-1;
              break;
             }
   ...
   case 7  : if ((ptr_us=MALLOC(nm*sizeof(SCSUINT16_COP)))==NULL)
             {
              ierr=-1;
              break;
             }
   ...
   case 8  : if ((ptr_uc=MALLOC(nm*sizeof(SCSUINT8_COP)))==NULL)
             {
              ierr=-1;
              break;
             }
...
int intscixstringb(char *fname,unsigned long fname_len)
{
  ...
  while(!checkwidth) {
    ...
    for (i = m3 -1;i >= 0;--i) {
      ...
      if (rect[2]>w) {
        checkwidth=0;
        /*sciprint("Le texte %s est trop large pour son gabari, de %f \n",C2F(cha1).buf,rect[2]-w);*/
        if ((ib>3)) {
          Str[i+ m3*(j-1)][ib-1]='\0';
          Str[i+ m3*(j-1)][ib-2]='.';
          Str[i+ m3*(j-1)][ib-3]='.';
