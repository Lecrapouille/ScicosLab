...
/*--------------------------------------------------------------------------*/
#ifdef WITH_TK
extern void C2F(tksynchro)(int *l);
#else
...
int C2F(parse)(void)
{
  ...
  static int returnFromCallbackExec = FALSE_;
 ...
 L15:
  ...
  if (C2F(iop).ddt == 4) {
    sprintf(tmp," parse  pt:%d rstk(pt):%d top: %d niv: %d err:%d",
	    Pt,r,Top_stack, C2F(recu).niv,Err);
    C2F(basout)(&io, &C2F(iop).wte,tmp, (long)strlen(tmp));
  }
     ...
     long int str_len;
{
  ...
  Pt = Max(Pt,0);Pts=Pt;
  Top_stack = Max(Top_stack,0);Tops=Top_stack;
 ...
 L89:
  ...
  if (Top_stack - Rhs + Lhs + 1 >= C2F(vstk).bot) {
    SciError(18);
    goto L9999;
  }
  goto L91;
 ...
 L95:
  ...
  if (C2F(com).fun >= 0) {
    if (Top_stack - Lhs + 1 > 0) {
      C2F(iset)(&Rhs, &c__0, &Infstk[Top_stack - Lhs+1], &c__1);
    }
    goto L90;
  }
