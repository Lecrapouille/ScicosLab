...
c     . hard predefined variables
         vsizg=1000
         call scimemp(vsizr+vsizg,stkptr)
         gtop=isiz+1
c     keeping track of end of variable 
         lstk(gtop)  =vsizr+1
c     start of global 
         lstk(gtop+1)=vsizr+1
         gbot=isizt
         lstk(gbot)=vsizr+vsizg
      else
c     .  memory allocation
         stk(1)=1.0d0
         offset=0
...
c     . COMPILER which was used for building 
c     scilab i.e returned by getenv('COMPILER') 
      vname = ' '
      vname(1:14) = "BUILD_COMPILER"
...
c     .  %t   : True boolean
      irep(1)=1
      call crebmatvar(true,k,1,1,irep)
      k=k+1
c     .  %f   : False boolean
      irep(1)=0
      call crebmatvar(false,k,1,1,irep)
      k=k+1
c     .  %eps : machine precision 
      rv(1)=dlamch('p')
      iv(1)=0.0
      call crematvar(eps,k,0,1,1,rv(1),iv(1))
...
c      call cvname(idloc,"%test",0)
      rv(1)=dlamch('p')
      iv(1)=0.0
c      call crematvar(idloc,k,0,1,1,rv(1),iv(1))
c      k=k+1
c     .  %io : standard input&output
      iov(1)=dble(rte)
      iov(2)=dble(wte)
      iv(1)=0.0
      call crematvar(io,k,0,1,2,iov,iv(1))
      k=k+1
c     .  %i : sqrt(-1)
      rv(1)=0.0
      iv(1)=1.0
      call crematvar(im,k,1,1,1,rv(1),iv(1))
      k=k+1
c     .  %e : exp(1)
      rv(1)=2.71828182845904530d+0
      iv(1)=0.0
      call crematvar(exp,k,0,1,1,rv(1),iv(1))
      k=k+1
c     .  %pi 
      rv(1)=3.14159265358979320d+0
      iv(1)=0.0
      call crematvar(pi,k,0,1,1,rv(1),iv(1))
      k=k+1
c     .  blanc 
      rv(1)=0.0
      iv(1)=0.0
      call crematvar(bl,k,0,1,1,rv(1),iv(1))
      k=k+1
c     
c
c     --------------
