...
#define ix ((int *)equiv_4)
 ...
 L10: /* Current opcode finished handle error, interruptions,...*/
  ...
  if (C2F(errgst).err1 != 0 ) {
	...
	if (Rstk[p]<=502 && Rstk[p]>=501){
	  ...
	  Lpt[6] = k;
	  C2F(recu).macr--;
	  if (Rstk[p-1]==909) Top_stack--; /* execed function*/
	}
 ...
 L28:
  if (Rhs == 0 && ((*istk(7 + lc) == -2)||(*istk(7 + lc) == -1)) && Fin == -1) {
    ...
    if (*istk(lc) != 29) {
      strcpy(C2F(cha1).buf,"Unexpected opcode, please report into the Scilab bug tracker.");
      SciError(9999);
      return 0;
    }
    lc += 10;
 ...
 L30:
  ...
  lc += 4;
  if (Fin == extrac) {
    C2F(isafunptr)(&Top_stack, id, &ifun, &ifin);
    if (ifun != 0) {
      --Top_stack;
      --Rhs;
      C2F(com).fun = ifun;
 ...
 L40:
  ...
  if (C2F(errgst).err1 <= 0) {
    ++Top_stack;
    if (C2F(cresmat)("run", &Top_stack, &c__1, &c__1, &n, 3L)) {
      C2F(getsimat)("run", &Top_stack, &Top_stack, &mm1, &nn1, &c__1, &
		    c__1, &lr, &nlr, 3L);
 ...
 L41:
  if (C2F(errgst).err1 <= 0) {
    ...
    ++Top_stack;
    if (C2F(cremat)("run", &Top_stack, &c__0, &c__1, &c__1, &lr, &lcc, 3L)) {
      *stk(lr) = *x;
    }
  }
  lc += 3;
 ...
 L46:
  ...
  Ids[1 + Pt * nsiz] = l0;
  Ids[2 + Pt * nsiz] = Lct[8];
  Ids[3 + Pt * nsiz] = Top_stack;
  Ids[4 + Pt * nsiz] = C2F(errgst).toperr;
  C2F(errgst).toperr = Top_stack;
 L47:
  lc = l0;
  if (Top_stack != Ids[3 + Pt * nsiz]) {
    SciError(115);
    goto L48;
  }
 ...
 L56:
  ...
  if (*istk(Pstk[Pt]) == 10) {
    ...
    i2 = Top_stack + 1;
    if (! C2F(vcopyobj)("run", &Top_stack, &i2,3L)) {
      return 0;
    }
    ++Top_stack;
  }
 ...
 L120:
  ...
  L121:  
	...
	(Rstk[Pt]==616 && Pstk[Pt] ==10)) Top_stack--;
 ...
 L130:
  Infstk[Top_stack] = 1;
  C2F(putid)(&C2F(vstk).idstk[Top_stack * nsiz - nsiz], istk(1 + lc));
  lc += 7;
  goto L10;
 ...
 L210:
  ...
  if (C2F(errgst).err1 <= 0) {
    ...
    nc = *istk(lc + 4 + n) - 1;
    ++Top_stack;
    il = Lstk[Top_stack] + Lstk[Top_stack] - 1;
    i2 = il + 5 + n + nc;
    Err = i2 / 2 + 1 - Lstk[Bot];
    if (Err > 0 || C2F(errgst).err1 > 0) {
      SciError(17);
 ...
 L252:
  ...
  if (lastindpos != Top_stack)   --Top_stack;
 ...
 L260:
  if (*istk(1 + lc) == 1) {
    /* | case */
    if (C2F(gettype)(&Top_stack) != sci_ints && Istrue(0)) {
      lc += *istk(2 + lc);
    }
  } else {
    /* & case */
