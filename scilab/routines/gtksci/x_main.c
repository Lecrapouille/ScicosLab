...
static void sci_gtk_init(gint argc, gchar *argv[]);
...
static void gtk_menu_force_ref(int x) 
{  
...
void C2F(realmain)()
{
  ...
  for ( i=0 ; i < argc ; i++) 
    {
      ...
      else if ( strcmp(argv[i],"-display") == 0) {/* display =argv[++i];*/ ++i;} 
  ...
  */
  ...
  /* cleaning */
#ifdef WITH_TK
  /* force deletion of TCL interpreter */
  CloseTCLsci();
#endif
  C2F(sciquit)();
}
...
void start_sci_gtk() {
  ...
  if ( i== 0 && GetBasic() == 1) 
	...
	exit(1);
      ...
      sci_gtk_init(argc,argv);
      SetNotBasic();
      /* we need also to initialize tk since sxevents will check events */
#ifdef WITH_TK
      initTCLTK();
#endif
    }
