...
int C2F(macr2tree) (char *fname)
{
  /* Returned value parameters */
  int m_pgrm_tlist = 1,n_pgrm_tlist = 6;
  char *pgrm_tlist[] = 
    {"program","name","outputs","inputs","statements","nblines"};
  ...
  int sz = 0; /* Size */
  int newinstr = 0; /* flag used to know if a new instruction 
		       has been created (1->TRUE) */
  ...
  /* Read all data */
  stkdata = (int *) stk(*Lstk(Top_stack));
  ...
  if((name[0]=(char *)CALLOC((nlgh+1),sizeof(char)))==NULL)
    {
      Scierror(999,"macr2tree: No more memory available\r\n");
      return 0;
    }
  ...
  /* Must be done before writing anything on stack or else we have to save Top_stack when entering this program */

  CvNameL(idstk(1,Top_stack),name[0],&job1,&namelgth);
  (name[0])[namelgth]='\0';
  ...
  if((data=(int *)CALLOC((codelength+ilt+1),sizeof(int)))==NULL)
    {
      Scierror(999,"macr2tree: No more memory available\r\n");
      return 0;
    }
  ...
  /* List header */
  /* Considering Top_stack is pointing last occupied place */

  /* Number of elements in list */
  cod_ind = ilt + 1;
  nbstat = complexity(data,&ilt,&codelength);

  Top_stack++; /* First free place */
  ...
  *Lstk(Top_stack+1) = sadr(il+3+nbstat);
  
  /* Error handling (S. Steer */
  if (*Lstk(Top_stack+1) >= *Lstk(Bot)) {
  ...
  for(k=1;k<=nbstat;k++)
    {
	  ...
	  /* jpc: add a test not to read out of range data indice */ 
 	  if(cod_sav==15 && (cod_ind + 1 <  codelength+ilt+1) && data[cod_ind+1]==29) /* EOL as the last component of a column concatenation */
	    {
	      /* a = ['a'
		 'b'
		 ] */
	      Top_stack--; /* EOL is erased */
	      last_eol_pos=-10; /* EOL position is erased */
	      newinstr=0; /* No new instruction created */
	      nbstat--; /* One statement deleted */
	    }
	  else if(cod_sav==15 && Top_stack!=TopSave+1) /* Column catenation with EOL after semi-colon */
	    newinstr=0;
	  else if(cod_sav==15) /* If EOL is not after semi-colon in catenation, it is ignored */
	    last_eol_pos=-10;

	  cod_ind++;
	  /* jpc 2010 : replace > by >= since cod_ind is supposed to 
	   * be used in data which is of size codelength+ilt+1
	   */
      ...
      if(TopSave!=Top_stack-1) {
	Scierror(999,"macr2tree: wrong Top_stack value %d instead of %d\r\n",Top_stack,TopSave+1);
	...
	return 0;
      ...
      sz = *Lstk(Top_stack+1) - *Lstk(Top_stack);

      *istk(il+2+k) = *istk(il+1+k) + sz ;

      Top_stack--;
      
      *Lstk(Top_stack+1) = *Lstk(Top_stack+2);
    }
...
static int CreateEOLList(void)
{
  ...
  if((eol[0]=(char *)CALLOC((strlen("EOL")+1),sizeof(char)))==NULL)
    {
      Scierror(999,"CreateEOLList: No more memory available\r\n");
      return 0;
    }
	  ...
	  while(icase > 1)
	    {
	      ...
	      if(icase==ncase && data[index0]==10)
      		TopSave_elseifsorcases = Top_stack; /* Saved to know how many cases have been written */
	      ...
	      while(*index<=endindex)
		{
	      ...
	      TopSave = Top_stack; /* Position on stack saved to get the number of instructions */
	      while(*index<=endindex)
		{
		  GetInstruction(data,index,nblines,&newinstr);
		  (*index)++;
		...
		TopSave_elseifsorcases = Top_stack; /* Saved to know how many elseifs have been written */
	    }
	  
	  nbelseifsorcases = Top_stack - TopSave_elseifsorcases;
...
static int CreateCsteTList(char *type,int *data,int *index)
{
  ...
  if(!strncmp(type,"emptymatrix",11))
      ...
      if((str=CALLOC(1,sizeof(char*)))==NULL)
	{
      ...
      if((str[0]=(char *)CALLOC((strlgth+1),sizeof(char)))==NULL)
	{
	  Scierror(999,"CreateCsteTList: No more memory available\r\n");
	  return 0;
	}
      if((int_str=(int *)CALLOC((strlgth+1),sizeof(int)))==NULL)
	{
	  Scierror(999,"CreateCsteTList: No more memory available\r\n");
	  return 0;
	}
...
static int CreateOperationTList(int *data,int *index)
{
  ...
  if((operator[0]=(char *)CALLOC((max_op_lgth+1),sizeof(char)))==NULL)
    {
      Scierror(999,"CreateOperationTList: No more memory available\r\n");
      return 0;
    }
  ...
  /*  Example: a=[1,2;
                  3,4] */
  if( (operator_index==26) && (last_eol_pos==Top_stack-2) )
    {
      /* Change operator */
      operator_index = 32;
  ...
  orig = Top_stack;
  dest = Top_stack - nb_operands - offset;
  VCopyObj("CreateOperationTList",&orig,&dest,20L);
...
static int CreateFuncallTList(char *fromwhat,int *data,int *index)
{
  ...
  if((funname[0]=(char *)CALLOC((nlgh+1),sizeof(char)))==NULL)
    {
      Scierror(999,"CreateFuncallTList: No more memory available\r\n");
      return 0;
    }
...
static int CreateEqualTList(char *fromwhat,int *data,int *index)
{
  ...
  if((operator[0]=(char *)CALLOC(4,sizeof(char)))==NULL)
    {
      Scierror(999,"CreateEqualTList: No more memory available\r\n");
      return 0;
    }
  ...
  if(!strncmp(fromwhat,"code29",6))  /* A code 29 was found in data */
      ...
      if(symbol==43) /* ; */
	{
	  ...
	  if((endsymbol[0]=(char *)CALLOC(2,sizeof(char)))==NULL)
	    {
	      Scierror(999,"CreateEqualTList: No more memory available\r\n");
	      return 0;
	    }
      ...
      else /* Nothing */
	{
	  ...
	  if((endsymbol[0]=(char *)CALLOC(1,sizeof(char)))==NULL)
	    {
	      Scierror(999,"CreateEqualTList: No more memory available\r\n");
	      return 0;
	    }
...
static int CreateCommentTList(int *data,int *index)
{
  ...
  (*index)++;   
  /* Memory allocation */
  if((text=(char *)CALLOC((strlgth+1),sizeof(char)))==NULL)
    {
      Scierror(999,"CreateCsteTList: No more memory available\r\n");
      return 0;
    }
