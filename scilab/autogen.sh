...
# libtool is named glibtool on mac osX 
...
(grep "^A[CM]_PROG_INTLTOOL" $srcdir/configure.in >/dev/null) && {
...
(grep "^A[CM]_PROG_XML_I18N_TOOLS" $srcdir/configure.in >/dev/null) && {
...
(grep "^A[CM]_GLIB_GNU_GETTEXT" $srcdir/configure.in >/dev/null) && {
...
do
  ...
  if test -f $dr/NO-AUTO-GEN; then
    ...
    ( cd $dr
      ...
      if grep "^A[CM]_GLIB_GNU_GETTEXT" configure.in >/dev/null; then
      ...
      if grep "^A[CM]_PROG_INTLTOOL" configure.in >/dev/null; then
        echo "Running intltoolize..."
        intltoolize --copy --force --automake
      fi
      if grep "^A[CM]_PROG_XML_I18N_TOOLS" configure.in >/dev/null; then
...
if test x$NOCONFIGURE = x; then
  ...
  && echo Now type \`make all\' to compile. || exit 1
else
