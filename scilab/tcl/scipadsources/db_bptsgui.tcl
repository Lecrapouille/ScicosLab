...
# display the breakpoints properties dialog
    ...
    #        better solution. The width of the columns handling comboboxes must
    ...
    #        each combobox, search for the longest item of the combobox, select it,
    #        grid the combobox, select back the item initially selected.Searching
    ...
    entry $cfr.expression$ID -font $textFont -takefocus 0\
            -width $expcolwidth
    combobox $cfr.conditiontype$ID -font $textFont -takefocus 0 \
            -width $ctycolwidth -editable false
    ...
    label $cfr.hitcount$ID -font $textFont -relief groove -takefocus 0 \
            -width $hctcolwidth
    combobox $cfr.hitcountcondition$ID -font $textFont -takefocus 0 \
            -width $bricolwidth -editable false
    ...
    pack $bptsgui.fb -expand 0 -fill x -pady 5 -side bottom
...
# more precisely inside the container scrolledframe of this gui
    ...
    global textFont
    global bptsguicondtypecombolistvar bptsguihitcountcondcombolistvar
    global bptsprops
    ...
    label $cfr.funline$ID -font $textFont -relief groove -takefocus 0 \
            -width $lincolwidth
    if {$linenumberstype ne "logical_ignorecontlines"} {
        $cfr.funline$ID configure -textvariable bptsprops($ID,funline)
    } else {
        $cfr.funline$ID configure -textvariable bptsprops($ID,funphysline)
    }
    ...
    # -textvariable is not used in the combobox, instead a separate procedure
    # is called through the -command option (see proc updatebptspropscondtype
    # for the good reasons)
    set bptsguicondtypecombolistvar [listlocalize $bptconditiontypes]
    combobox $cfr.conditiontype$ID -font $textFont \
            -listvar bptsguicondtypecombolistvar \
            -command "updatebptspropscondtype $ID" \
            -editable false -width $ctycolwidth
    $cfr.conditiontype$ID configure -value [mc $bptsprops($ID,conditiontype)]
    ...
    label $cfr.hitcount$ID -font $textFont -relief groove -takefocus 0 \
            ...
            -width $hctcolwidth
    # same comment for this combobox as for $cfr.conditiontype$ID
    set bptsguihitcountcondcombolistvar [listlocalize $bpthitconditions]
    combobox $cfr.hitcountcondition$ID -font $textFont \
            -listvar bptsguihitcountcondcombolistvar \
            -command "updatebptspropshitconds $ID" \
            -editable false -width $bricolwidth
    $cfr.hitcountcondition$ID configure -value [mc $bptsprops($ID,hitcountcondition)]
    ...
    # but not for check/combo boxes (because blinking delays the update)
...
proc updatebptspropscondtype {ID comboname newconditiontype} {
# update bptsprops($ID,conditiontype)
# this proc is called whenever an item is clicked in the condition type
# combobox of the bpts gui
# one could have obtained the same effect through -textvariable in the
# combobox definition, but then one would have had to deal with the locale
# whenever bptsprops($ID,conditiontype) is used thoughout the code
# with this approach only the display in the combobox is localized, and
...
# displayed ones)
# note: parameter comboname is not used but is provided by the -command
#       mechanism of the combobox - it cannot be removed from the list
#       of parameters
    global bptsprops bptconditiontypes

    set pos [lsearch -exact [listlocalize $bptconditiontypes] $newconditiontype]
    # $pos == -1 should not happen, i.e. the expression condition type
    # newly selected in the listbox of the combobox should always be
    # found in the localized list of expression condition types

    set bptsprops($ID,conditiontype) [lindex $bptconditiontypes $pos]
}

proc updatebptspropshitconds {ID comboname newhitcondition} {
...
# see proc updatebptspropscondtype about why -textvariable is not
# used in the combobox definition and why this proc is used instead
...
#       of parameters
    ...
    # $pos == -1 should not happen, i.e. the hit condition type
    # newly selected in the listbox of the combobox should always be
    # found in the localized list of hit condition types
...
# a dialog explaining a bit the vocabulary about breakpoints conditions
    ...
    set mes [mc "A breakpoint is identified by its location, i.e. its position (a line number) in a function."]
    append mes "\n\n"
    append mes [mc "When the execution process encounters an enabled breakpoint, then this breakpoint is said to be reached."] 
    append mes "\n\n"
    append mes [mc "When the breakpoint is reached and its associated conditional expression is true (or changed), then the breakpoint is said to be hit."]
    append mes "\n\n"
    append mes [mc "The hit count is the number of times the breakpoint has been hit."]
    append mes "\n\n"
    append mes [mc "Execution will stop at a breakpoint if the hit count satisfies the selectable break condition."]
...
# show the piece of code that contains the breakpoint identified by $ID
    global bptsprops
    set ma $bptsprops($ID,funname)
    set li $bptsprops($ID,funline)
    after idle "blinkline [condloglinetophyslineinfun $ma $li] $ma 1 ; focus $focuswidget"
}
...
proc switchdisplayedlinenumbersinbptsgui {} {
# arrange for logical line numbers or physical line numbers in functions
# to be displayed by the breakpoints gui
    if {[istoplevelopen bptsgui]} {
        showbptgui_bp
    }
}
