...
if {$debuglog} {
    ...
    colorize remalltags remalltags_strcomincompat tagcontline \
    ...
    backgroundcolorizeuserfun docolorizeuserfun docolorizeuserfunwhenidle \
    backgroundcolorizeuservar docolorizeuservar docolorizeuservarwhenidle \
    colorizetag \
    ...
    update_bubble_panetitle update_bubble_uservar \
    displaybusystate \
    istoplevelopen \
    escapequotes \
    updatelinenumbersmargin updatelinenumbersmargin_physical \
    updatelinenumbersmargin_logical doupdatelinenumbersmargin_logical \
    updatemodifiedlinemargin \
    setTextAnchor_scipad \
    showmessagemissingtkdndpackage \
    timeintervalsecs \
    show_matching_char show_matching_char_at_cursor show_matching_char_at_mouse \
    getfindbsregexpcorrbscharandsearchdir getindofsymbol2 \
    isscilabbusy \
    ]
#    set excludedScipadprocs [list displaybusystate istoplevelopen]
    ...
    proc __log__menues {men} {
        foreach w [winfo children $men] {
            ...
            if {[$w type $i] == "cascade"} {
                catch {__log__menues $w}
            }
        }
    }
    __log__menues $pad.filemenu

    # for each Scipad binding, this surrounds the existing binding with log info
    proc __log__getchildren {w} {
        set chil [winfo children $w]
        set allch $chil
        foreach ch $chil {
            foreach wid [__log__getchildren $ch] {
                if {[winfo class $wid] ne "Menu"} {
                    lappend allch $wid
                } else {
                    # nothing: menues are logged through another mechanism
                    # showing more information and highlighting in the log
                    # that a command was clicked
                }
            }
        }
        return $allch
