...
proc configurefoo_bp {} {
    ...
    global listboxinput listboxinputval listboxscrolly funnamecombo buttonAddc
    ...
    set funnamecombo $conf.f.f1.combo
    combobox $funnamecombo -font $textFont -listvar funnames -editable false
    set buttonObtain $conf.f.f1.buttonObtain
    eval "button $buttonObtain [bl "&Obtain"] \
            -command \"Obtainall_bp\" \
            -font \[list $menuFont\] "
    pack $conf.f.f1.label $funnamecombo $buttonObtain \
         -side left -padx 4
    pack configure $conf.f.f1.combo -expand 1 -fill x
    pack $conf.f.f1 -expand 1 -fill x
    ...
    $buttonAddc configure -command {if {[$funnamecombo get] != ""} {Addarg_bp $conf $buttonAddc $listboxinput $listboxinputval}}
...
#    bind $conf <Return> "OKconf_bp $conf"
    bind $conf <Return> {if {[$funnamecombo get] != ""} {Addarg_bp $conf $buttonAddc $listboxinput $listboxinputval}}
    bind $listboxinput <Double-Button-1> {if {[$funnamecombo get] != ""} {Addarg_bp $conf $buttonAddc $listboxinput $listboxinputval}}
#    bind $conf <Escape> "Cancelconf_bp $conf"
    ...
    bind $listboxinput <ButtonRelease-3> {dropitem_bp $listboxinput $listboxinputval $funnamecombo $itemindex %y}
    ...
    # populate the combobox with function names,
    # and the listboxes for variable names and values
    set oppar [string first "\(" $funnameargs]
    set funname [string range $funnameargs 0 [expr {$oppar - 1}]]
    if {$funname ne ""} {
        $funnamecombo configure -value $funname
    } else {
        $funnamecombo configure -value [lindex $funnames 0]
        set funname [$funnamecombo get]
    }
    # don't do that before since the -command gets executed now
    $funnamecombo configure -command selectednewiteminfunctioncombolistbox
...
proc selectednewiteminfunctioncombolistbox {comboname selecteditem} {
    global listboxinput listboxinputval funvars funvarsvals
    $listboxinput delete 0 [$listboxinput size]
    $listboxinputval delete 0 [$listboxinputval size]
    set funname $selecteditem
    if {[info exists funvars($funname)]} {
        foreach var $funvars($funname) {
            $listboxinput insert end $var
            $listboxinputval insert end $funvarsvals($funname,$var)
...
proc dropitem_bp {leftwin rightwin callwidget dragind droppos} {
# may be called from the configure dialog, or from the watch window
# the difference is noticed since:
#    $callwidget == "" : called from the watch window
#    $callwidget <> "" (value $funnamecombo) : called from the configure box
...
proc OKconf_bp {w} {
    global listboxinput listboxinputval funnamecombo
    global funnameargs
    set funname [$funnamecombo get]
    if {$funname ne ""} {
...
# argument names are gathered and displayed in the configure box
    global funnamecombo listboxinput listboxinputval funnames funvars funvarsvals
    global funsinbuffer
    ...
    if {[lindex $funsinfo 0] ne "0NoFunInBuf"} {
    ...
    if {!$treatassce} {
        ...
        $funnamecombo list delete 0 end
        set listboxvalues ""
        $listboxinput delete 0 [$listboxinput size]
        $listboxinputval delete 0 [$listboxinputval size]
        foreach {funname funline precfun} $funsinfo {
            ...
            set escfunname [string map {"\?" "\\\?"} $funname]
            if {[lsearch $listboxvalues $escfunname] == -1} {
                lappend listboxvalues $funname
            }
            set oppar [string first "\(" $funline]
            set clpar [string first "\)" $funline]
            set listvars [string range $funline [expr {$oppar + 1}] [expr {$clpar - 1}]]
        ...
        # populate the listbox of the function name combobox
        # note: this   set funnames $listboxvalues   should NOT be done
        #       otherwise (in case there is more than 10 items) the scrollbar
        #       will be missing (combobox bug!)
        # besides, since -listvar funnames in the combobox definition,
        # there is no need to set also funnames content
        foreach elt $listboxvalues {$funnamecombo list insert end $elt}
        $funnamecombo configure -value $funtoset
        selectednewiteminfunctioncombolistbox $funnamecombo $funtoset
        $listboxinput see 0
        set debugassce false
        ...
        # exec all non level zero code from all buffers, and do this
        # now before wrapping so that functions defined in mixed .sci/.sce
        # file will be exec'ed, which is needed so that functions that are
        # ancillaries to the level zero code become known by Scilab, which is
        # required at debug run time
        # another non level zero code exec will be done later in proc execfile_bp
        # but this will exec the wrapped function (not the function ancillaries
        # of the level zero code) so that Scilab knows how to execute it for
        # debugging
        if {[isscilabbusy 5]} {return}
        set execresult [execallnonlevelzerocode]
        if {$execresult == -1} {
            # abort .sce case configuration
            set mes [mc "Could not send non level zero code to Scilab during configuration of the debugger for .sce file."]
            set tit [mc "Error"]
            tk_messageBox -message $mes -icon error -title $tit
            return
        }
        if {$execresult == 4} {
            # code contains pause or abort, exec'ing it would render the interpreter unstable
            # the message box was already displayed in proc execfile
            return
        }
        ...
        # to adjust $lastlinenumber in proc getlinenumbersranges
        ...
        # adjustment of $lastlinenumber in proc getlinenumbersranges
        ...
        if {!$mflag} {
            ...
            after idle [list catch [list resetmodified $textarea "clearundoredostacks"]]
        }
...
# all the textareas
# also, remove any existing breakpoint from the level zero code
# (but keep breakpoints in functions (case of mixed .sce/.sci)
    global listoftextarea debugassce
    global funnames funnameargs linenumbersranges

    # if the debug in progress was not a .sce debug, do nothing
    if {!$debugassce} {return}

    # remove breakpoints from level zero code
    removeallbpt_level0_scipad_bp
    ...
    foreach w [filteroutpeers $listoftextarea] {
        ...
        $w configure -undo 1
        if {!$mflag} {
            after idle [list catch [list resetmodified $w "clearundoredostacks"]]
        }
    ...
    # cancel)
    unset -nocomplain linenumbersranges
