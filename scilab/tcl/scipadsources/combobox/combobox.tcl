#########################################################################################
#
# Code edited by F. Vogel, 2015
#
# 1. (no code change related to this point)
#    Bug found:
#        ::combobox::combobox .co -listvar myvar
#        pack .co
#        set myvar [list V1 V2 V3 V4 V5 V6 V7 V8 V9 V10 V11 V12 V13 V14 V15]
#    and now open the dropdown listbox: the scrollbar should be visible since there are
#    more than 10 elements, but it is missing!
#    I think a  ::combobox::HandleScrollbar  must be missing somewhere, but where...?
#    In fact when changing the content of the listvariable, no particular event is fired
#    to the combobox whereas it would be needed to launch  ::combobox::HandleScrollbar
#    Workaround 1 (for a combobox with fixed content):
#       Define the content of the listvar variable before creating the combobox:
#           set myvar [list V1 V2 V3 V4 V5 V6 V7 V8 V9 V10 V11 V12 V13 V14 V15]
#           ::combobox::combobox .co -listvar myvar
#           pack .co
#    Workaround 2 (for a combobox with variable content):
#       Instead of setting myvar directly in one statement, insert elements one by one
#       through the list insert command:
#           .co list delete 0 end
#           set varct [list V1 V2 V3 V4 V5 V6 V7 V8 V9 V10 V11 V12 V13 V14 V15]
#           foreach elt $varct {.co list insert end $elt}
#
# 2. (no code change related to this point)
#    Mysterious flashing behaviour triggered in UpdateVisualAttributes:
#    I thought about commenting out the line:
#         $widgets(frame)   configure -background $background
#    because it makes the combobox flash whenever the mouse enters/leaves it when
#    the -relief option is used, which is an issue when placing the combobox in
#    a toolbar (the -relief option is modified when hovering the combobox, and
#    again when the mouse goes away)
#    The problem is not fully understood, however it can be reproduced in wish with
#    the following simple script:
#        source "combobox.tcl"
#        ::combobox::combobox .co 
#        pack .co
#        .co configure -background red    ;  # FLASH!!
#    I have traced this to run in turn:
#        ::combobox::WidgetProc .co configure -background red
#          ==> ::combobox::Configure .co -background red
#                ==> UpdateVisualAttributes .co
#    and in the latter proc the flash is triggered by line:
#        $widgets(frame)   configure -background $background
#    At this point in the code  $widgets(frame)  is  ::combobox::.co::.co
#    The following flashes then the same and is more direct:
#        ::combobox::.co::.co configure -background red
#    but this time it does not involve any proc from the combobox package.
#    Binding? Couldn't find a culprit. Removed all of them and it still happens.
#    "update" commands? It still happens when all are commented.
#    Clueless...
#    Therefore I could perhaps just comment the guilty line hoping the side effect
#    will not be seen (possibly wrong colorization of the background of the very
#    tiny frame containing the entry and the label (button) making up the combobox)
#    BUT this is not enough since there are other  $widgets(frame) configure
#    commands in combobox.tcl, and these cannot be commented otherwise the feature
#    they provide would then be missing...
#    I have asked the question in comp.lang.tcl:
#      http://groups.google.com/group/comp.lang.tcl/browse_thread/thread/2c93cbf91ac8b674
#    but did not get an answer.
#    Conclusion: I did nothing...
#
# 3. Fixed bug (typo) in proc ::combobox::WidgetProc
#    oldFocus and oldGrab were mixed up, resulting in wrong focus restore after
#    closure of the dropdown listbox - no error was throw since a catch is there!
#
#########################################################################################

##############################################################################
###
### Combobox widget - Version 2.3 - From Bryan Oakley
###
##############################################################################
