...
# hits Cancel)
    ...
    bind $adda <Return> "$adda.f.f9.buttonOK invoke"
    bind $adda <Escape> "$adda.f.f9.buttonCancel invoke"
...
# Close the add argument dialog and take the values of its fields into account
    global unklabel
    global argname argvalue
    global funnamecombo funvars funvarsvals
    ...
    if {$argname != ""} {
        ...
        # 4. Attempts to update a nolocal auto watched variable must be killed.
        # Note that in principle only attempts to redefine nolocals must be
        # killed, but not necessarily attempts to redefine other non editable
        # variables. This is because redefining nolocals would mess the
        ...
        # however not true for local non editable variables that could
