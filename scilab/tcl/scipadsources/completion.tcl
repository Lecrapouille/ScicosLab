...
# the output list is sorted by alphabetical order of completions
    ...
    # user functions must be colorized in real time NOW (and not when idle)
    # because the $mode array must be up-to-date concerning function names
    # when getting the possible completion candidates below
    # if this colorization is not done, then it's easy to propose obsolete
    # function names:
    #    open source of, say  abcd.sci
    #    wait a bit for user function colorization (the corresponding
    #      checkbox must be set in the options menu)
    #    close the file
    #    without waiting (don't give a chance for dowhenidle to execute
    #    the pending docolorizeuserfun), open source of again on  abcd
    #    ----> both the libfun abcd.sci and the user function abcd are
    #          proposed, but the latter has already been closed. Selecting
    #          the latter would trigger a bug
    docolorizeuserfun
...
#   if N>15, ring a bell
    ...
    # <TODO> avoid hard definition of scilabnameschars here, replace by use of regexp with $sncc1RE
    ...
    if {$nbcompl == 0} {
        ...
        foreach tag $tags {
            switch -- $tag {
                ...
                uservar {set col $USERVARCOLOR}
                Modelica_keyword {set col $MODELICAKEYWORDCOLOR}
                Modelica_introp  {set col $MODELICAINTROPCOLOR}
