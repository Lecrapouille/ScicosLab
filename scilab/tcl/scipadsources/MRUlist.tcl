...
# one line down and insert $encod at the top
    ...
    if {!$present} {
        ...
        if {$nbrecentencodings == 0} {
            incr rec1ind
            $pad.filemenu.options.opencloseexit.encodings insert $rec1ind separator
        }
        # update the encoding menu
        if {$nbrecentencodings < $maxrecentencodings} {
            ...
            $pad.filemenu.options.opencloseexit.encodings insert $rec1ind radiobutton \
...
# update labels of recent encodings entries preceded by a number
    ...
    for {set i 0} {$i<$nbrecentencodings} {incr i} {
        ...
        set ind [expr {$rec1ind + $i}]
        $pad.filemenu.options.opencloseexit.encodings entryconfigure $ind \
                   -label $lab \
                   -command "changeencoding" \
                   -value [lindex $listofrecentencodings $i] -variable currentencoding
        if {$i<9} {
            $pad.filemenu.options.opencloseexit.encodings entryconfigure $ind -underline 0
        } else {
            $pad.filemenu.options.opencloseexit.encodings entryconfigure $ind -underline -1
        }
    }
}
...
proc BuildInitialRecentEncodingsList {} {
    ...
    for {set i 0} {$i<$nbrecentencodings} {incr i} {
        ...
        $pad.filemenu.options.opencloseexit.encodings add radiobutton \
                   -label $lab \
                   -command "changeencoding" \
                   -value [lindex $listofrecentencodings $i] -variable currentencoding
        if {$i<9} {
            set ind [$pad.filemenu.options.opencloseexit.encodings index end]
            $pad.filemenu.options.opencloseexit.encodings entryconfigure $ind \
                   -underline 0
        }
    }
...
# in the options/encodings menu is changed by the user
    ...
    if {$maxrecentencodings >= [llength $listofrecentencodings]} {
        ...
        $pad.filemenu.options.opencloseexit.encodings delete $firstind $lastind
        ...
        if {$maxrecentencodings == 0} {
            # remove the separator
            $pad.filemenu.options.opencloseexit.encodings delete $firstind
        }
    }
}
