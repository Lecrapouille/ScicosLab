...
# Create the watch window
    ...
    set checkboxautowatchnolocals $watch.f.vpw.f2.f2l.autowatchnolocals
    eval "checkbutton $checkboxautowatchnolocals [bl "Au&to watch nolocals too"] \
            -variable autowatchnlo \
            -command \"manageautowatchnlo_bp\" \
            -onvalue \"true\" -offvalue \"false\" \
            -font \[list $menuFont\] \
            -anchor w -borderwidth 1 -pady 0"
    if {!$autowatchloc} {
        $checkboxautowatchnolocals configure -state disabled
    }
    if {$dockwatch} {$checkboxautowatchnolocals configure -underline -1}
    ...
    grid $checkboxautowatchnolocals -row 4 -column 0 -sticky w  -pady 4
    ...
    bind $watch <Alt-[fb $checkboxautowatchnolocals]>      "$checkboxautowatchnolocals invoke"
        ...
        if {[info exists watchvars]} {
            ...
            foreach var $watchvars {
                ...
                if {[isvareditable $var]} {
                    ...
                    if {[isnolocalautowatchvar $var]} {
                        # non editable because a nolocal
...
# switch on or off the local variables automatic display in the watch window
    manageautowatchlocnlo_bp loc
}

proc manageautowatchnlo_bp {} {
# switch on or off the nolocal variables automatic display in the watch window
    manageautowatchlocnlo_bp nlo
}

proc manageautowatchlocnlo_bp {locornolo} {
# switch on or off the local and/or nolocal variables automatic display
# in the watch window
    global watch autowatchloc autowatchnlo
    global callstackfuns
    if {[istoplevelopen watch]} {
        if {$locornolo == "loc"} {
            if {$autowatchloc} {
                $watch.f.vpw.f2.f2l.autowatchnolocals configure -state normal
            } else {
                $watch.f.vpw.f2.f2l.autowatchnolocals deselect
                $watch.f.vpw.f2.f2l.autowatchnolocals configure -state disabled
            }
            set checkboxtocheck $autowatchloc
            set opt "nolocalstoo"
        } else {
            set checkboxtocheck $autowatchnlo
            set opt "nolocalsonly"
        }
...
# the local and nolocal variables of function $curfun
...
# nolocal (yes, not global, see bug 2161 and git commit 
# 75ad575cef2ec89c30cf9b3fb548c625d4bf0529 )
# and local variables of the function whose name is the first item
# from $callstackfuns
...
# this is compatible with both trunk and BUILD4 branches
    ...
    set comm7     "TCL_EvalStr(\"set varsforautowatch(nolocals) \[escapespecialchars {\"+strcat(varsforautowatch_db(3),\" \")+\"}\]\",\"scipad\");"
    ...
    set commD     "TCL_EvalStr(\"set varsforautowatch(nolocals) \"\"\"\"\",\"scipad\");"
    set commE "end;"
...
# if $whichautos == "nolocalsonly" : nolocals
# if $whichautos == "nolocalstoo"  : in, out, locals, nolocals
# if $whichautos == "all", it depends on the "Auto (nolocals too)" checkbox status:
#     if $autowatchnlo is true    : in, out, locals, nolocals
#     if $autowatchnlo is false   : in, out, locals
    global varsforautowatch autowatchnlo
    global watchvars watchvarsprops

    if {$whichautos != "nolocalsonly"} {
        set listoftypes [list in out locals]
    } else {
        set listoftypes [list ]
    }
    if {$autowatchnlo || $whichautos == "nolocalsonly" || $whichautos == "nolocalstoo"} {
        lappend listoftypes nolocals
    }
...
# checked the corresponding "Auto add locals" checkbox
    global watch autowatchloc autowatchnlo
    if {$autowatchloc} {
        set saved_autowatchnlo $autowatchnlo
        ...
        set autowatchnlo $saved_autowatchnlo
    }
}

proc isnolocalautowatchvar {var} {
# return true if $var is a purely nolocal auto watched variable,
# and false otherwise
    global varsforautowatch
    if {![info exists varsforautowatch]} {
        return false
    } elseif {[lsearch -exact $varsforautowatch(nolocals) $var] == -1} {
        set retval false
    } else {
...
# and if it's not a nolocal variable
    global watchvarsprops
    if {$watchvarsprops($varname,editable) && ![isnolocalautowatchvar $varname]} {
        return true
    } else {
        return false
    }
...
proc getonewatchvarfromshell_forbubble {hvar} {
# Get one single watched variable content by getting its value from Scilab
# This is used to update the bubble when hovering over a user variable
    global unklabel
    # certain special characters are allowed in Scilab names,
    # these must be escaped
    set escwvar [escapespecialchars $hvar]
    set fullcomm ""
    set comm1 "if ext_exists(\"$hvar\"),"
    set comm2 "  \[db_svar,db_tysi\]=FormatStringsForWatch($hvar);"
    set comm3 "  if length(db_svar) < 150 then"
    set comm4 "    TCL_EvalStr(\"set bubbletextuservar \"\"$escwvar: \"+db_svar+\"\n\"+db_tysi+\"\"\"\",\"scipad\");"
    set comm5 "  else"
    set comm6 "    TCL_EvalStr(\"set bubbletextuservar \"\"$escwvar: \"+part(db_svar,1:150)+\"...\n\"+db_tysi+\"\"\"\",\"scipad\");"
    set comm7 "  end;"
    set comm8 "else"
    set comm9 "  TCL_EvalStr(\"set bubbletextuservar \"\"$escwvar: $unklabel\"\"\",\"scipad\");"
    set commA "end;"
    set fullcomm [concat $comm1 $comm2 $comm3 $comm4 $comm5 $comm6 $comm7 $comm8 $comm9 $commA]
    ScilabEval_lt $fullcomm "seq"
}
...
# or because it is a nolocal) or a variable whose value is $unklabel is not part
...
#   . Scipad does not transform nolocals into locals, which would otherwise mess
...
# display the line in the file pointed by the mouse in the call stack area
    ...
    if {[getdbstate] == "DebugInProgress"} {
        ...
        # see note on $linetogo below
        set linetogo   [lindex $callstacklines [expr {$lin - 1}]]
    } else {
        # no debug in progress
        # either there was no error (the call stack area is empty), and in
        # this case $errfunc is "" and $errline is 0
        ...
        # see note on $linetogo below
        set linetogo   $errline
    }
    # note on $linetogo
    # $linetogo is physical or logical (same "type" as $callstacklines) depending on
    # the Scilab environment around (see function FormatWhereForWatch)
    ...
    if {$keywtype != "error"} {
        ...
        ScilabEval_lt "TCL_EvalStr(\"blinkline $linetogo $nametoopen 1\",\"scipad\")" "seq"
    } else {
...
#    - variable names detection (for colorization and completion)
