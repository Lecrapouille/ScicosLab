...
//**
  ...
  //@@ %scicos_action set the default move
  //@@ %t : free move
  //@@ %f : smart move
...
//    if chngwin & or(btn==[0,1]) then btn=3,end  // in case of window
    ...
    elseif (btn>=32) & (btn<288)
      //Search in %scicos_short the assiocated menu
    ...
    elseif (btn==1000) then //CESTPASUNPEUGROS1000
                            //POURUNSMARTMOVE??
      if %scicos_action then
        cmd='Cmenu = '"Smart Move'"'
      else
  ...
  elseif btn > 31 then //** [CTRL] + [.] combination
    ...
    //** ----- Mouse + Keyb. combos ---------------------------  
    elseif (btn==1003) | (btn==2003)| (btn==2000) then  //** [CTRL]+[Left btn click] OR [Shift]+[Left btn click]
