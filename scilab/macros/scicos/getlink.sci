...
// Last Update 15 Dec 2008 Fady: Adding buses and boolean type
...
//      port_number=k,//out port
  ...
  while %t do //loop on link segments
    ...
    while 1 do //** infinite loop
      ...
      if with_gtk() then
        if exitok then
          ...
          if or(rep(3)==[0,2,3,5,-100]) then break,end
          if rep(3)==-1 then nb=nb+1, end
          if nb>10 then exitok=%t, end
        end
      else
    ...
    if kto<>[] then //new point designs the "to block"
      ...
      if typi==1 |typi==3 then // regular and buses input port
        port_number = k ;
  ...
  d_size = p_size(1) - o_size(1) ;             //** at the end of this "Link" operation
  drawlater() ;
  if d_size > 0 then
    gh_compound_delete = glue(gh_curwin.children.children(1:d_size) );
    delete (gh_compound_delete); //** delete the object
  ...
  if fromsplit then //link comes from a split
    ...
    gh_curwin.children.children(gh_ks).children(1).data = [link1.xx , link1.yy] ; //** update the graphics datastructure
    link1_color = gh_curwin.children.children(gh_ks).children(1).foreground     ; //** save the color
    link1_thickness= gh_curwin.children.children(gh_ks).children(1).thickness   ; // saving the thickness of the link will be used in buses (Fady)
    ident = link1.id;
    if ident==[] | ident=='drawlink' then
      ident=''
    end
    lk.id = ident

    font_style=gh_curwin.children.children(gh_ks).children($).font_style;
    font_size=gh_curwin.children.children(gh_ks).children($).font_size;
    font_color=gh_curwin.children.children(gh_ks).children($).font_foreground;
    visible=gh_curwin.children.children(gh_ks).children($).visible;
    rect=[min(link1.xx)+(max(link1.xx)-min(link1.xx))/2 min(link1.yy)+(max(link1.yy)-min(link1.yy))/2];
    rectstr = stringbox(ident, rect(1,1), rect(1,2), 0,font_style,font_size);
    w=(rectstr(1,3)-rectstr(1,2)) * %zoom;
    h=(rectstr(2,2)-rectstr(2,4)) * %zoom;
    gh_curwin.children.children(gh_ks).children($).data=[rect(1,1) - w / 2, rect(1,2) - (h*1.1)]
      //---------------------------
    ...
    rect=[min(link2.xx)+(max(link2.xx)-min(link2.xx))/2 min(link2.yy)+(max(link2.yy)-min(link2.yy))/2];
    w=(rectstr(1,3)-rectstr(1,2)) * %zoom;
    h=(rectstr(2,2)-rectstr(2,4)) * %zoom;
    xstringb(rect(1,1) - w / 2, rect(1,2) - (h*1.1) , ident , w, h,'fill');
    gh_curwin.children.children(1).font_style=font_style;
    gh_curwin.children.children(1).font_size=font_size;
    gh_curwin.children.children(1).font_foreground=font_color;
    gh_curwin.children.children(1).visible=visible;

    xpoly(link2.xx , link2.yy);
    gh_curwin.children.children(1).foreground = link1_color ;
    gh_curwin.children.children(1).thickness = link1_thickness;
    glue(gh_curwin.children.children(2:-1:1)); //** create the compound :)
...
//**----------------------------------------------------------------------------------
  ...
  if movedblock<>[] then
    ...
    gh_k=get_gri(movedblock,o_size(1))
