...
function txt=gen_outline_all(list_of_scistruc,list_of_scifunc,listfABCD)
  ...
  txt=[txt;
       "  <CHAPTER eng=""Scicos Data Structures"" fr=""Structures de donnée Scicos"">"]
  ...
  txt=[txt;
       "  <CHAPTER eng=""User''s manual"" fr=""Manuels utilisateur"">";]
  for i=1:size(list_of_user_manual,1)
     if list_of_user_manual(i,2)=='block' then
       txt=[txt;
            "   <BLK varpath="""" name="""+basename(list_of_user_manual(i,2))+"""></BLK>"]
     else
       txt=[txt;
            "   <SCI varpath="""" name="""+basename(list_of_user_manual(i,2))+"""></SCI>"]
     end
  end
  txt = [txt;
         "  </CHAPTER>"]
...
//gen_outline_pal
//##13/07/08 : Alan implements palette in palette
//##           full recursivity not yet implemented : TOBEDONE
function txt=gen_outline_pal2(listf)
  txt=[];
  if type(listf)==10 then
    txt='<PAL varpath=""palpath"" name="""+listf(find(listf(:,3)=='pal'),2)+""">";
    ...
    txt='<PAL varpath=""palpath"" name="""+listf(1)(1,2)+""">";

    for i=2:lstsize(listf)
      if listf(i)(1,3)=='pal' then
        txt=[txt;
             '<PAL varpath=""palpath"" name="""+listf(i)(1,2)+""">"];
        for j=2:size(listf(i),1)
          txt=[txt;
               "   <BLK varpath="""" name="""+basename(listf(i)(j,2))+"""></BLK>"];
        end
        txt=[txt;
             "</PAL>"];
      else
        txt=[txt;
             "   <BLK varpath="""" name="""+listf(i)(1,2)+"""></BLK>"];
      end
    end
...
function tt=generate_scs_outline()
 tt = ["<WHATIS>";
              ...
              "    <SCI varpath=""autopath"" name=""scicos_codegeneration.sci""></SCI>"
              "    <SCI varpath=""autopath"" name=""lincos.sci""></SCI>"
              "    <SCI varpath=""autopath"" name=""steadycos.sci""></SCI>"
              "    <SCI varpath=""autopath"" name=""model2blk.sci""></SCI>"
              "    <SCI varpath=""autopath"" name=""callblk.sci""></SCI>"
              "  </CHAPTER>"
              "";
              "  <CHAPTER eng=""Editor"" fr=""Editeur"">";
              "    <SCI varpath="""" name=""Menu_entries""></SCI>"
              "    <SCI varpath="""" name=""Setup_Scicos""></SCI>"
 ...
 tt = [tt;
              ...
              ""
              "  "+ gen_outline_pal(listf_of_iter);
              ""
              "  "+gen_outline_pal(listf_of_modelicablk);
              ""
              "  "+gen_outline_pal(listf_of_lut);
              ""
  ...
  tt = [tt;
               ...
               "  </CHAPTER>"
               ""
               "  <CHAPTER eng=""Use of Modelica"" fr=""Utilisation de Modelica"">"
               "    <SCI varpath="""" name=""ModelicaBlocks""></SCI>"
               "  </CHAPTER>"
               ""
               "  <CHAPTER eng=""Initialisation GUI of Scicos"" fr=""GUI d''initialisation Scicos"">"
               "    <SCI varpath="""" name=""iGUI""></SCI>"
               "  </CHAPTER>"
               ""
               "  <CHAPTER eng=""Scicos Code Generation"" fr=""Generation de code Scicos"">"
               "    <SCI varpath="""" name=""CodeGeneration""></SCI>"
               "  </CHAPTER>"
               ""
               ...
               "       <SCI varpath=""autopath"" name=""coserror""></SCI>"
               "     </SECTION>";
               "   </SUBCHAPTER>";
               "  </CHAPTER>"
               ""
               ...
               "    <SCI varpath=""utilpath"" name=""add_palette""></SCI>"
               "    <SCI varpath=""utilpath"" name=""create_palette""></SCI>"
               "    <SCI varpath=""utilpath"" name=""create_gif""></SCI>"
               "    <SCI varpath=""utilpath"" name=""get_scicos_version""></SCI>"
               "    <SCI varpath=""utilpath"" name=""getscicosparam""></SCI>"
               "    <SCI varpath=""utilpath"" name=""setscicosparam""></SCI>"
               "    <SCI varpath=""autopath"" name=""var2vec""></SCI>"
               "    <SCI varpath=""autopath"" name=""vec2var""></SCI>"
               "    <SCI varpath=""autopath"" name=""data2sig""></SCI>"
               "    <SCI varpath=""autopath"" name=""sig2data""></SCI>"
               "    <SCI varpath=""autopath"" name=""SetString""></SCI>"
               "  </CHAPTER>"
               ""
               "  <CHAPTER eng=""Scicos Data Structures"" fr=""Structures de donnée Scicos"">"
...
function gen_scs_editor_help(typdoc,%gd)
   ...
   for i=1:size(%gd.lang,1)
      ...
      //** generate tex head
      head_tex=get_head_tex(["","Setup_Scicos","sci"],typdoc,i,%gd)
      //** change title of html head
      if %gd.lang(i)=='fr' then
         head_tex=strsubst(head_tex,'Fonction Scilab','Editeur Scicos')
      elseif %gd.lang(i)=='eng' then
      ...
      name=get_extname(["","Setup_Scicos","sci"],%gd) 
      ...
      mputl(txt_setupscicos,%gd.lang(i)+...
            '/'+name+...
            '/'+name+'.tex');
...
//gen_scs_modelica : main generator for scitexgendoc of tex files
//for the list_of_modelica
function gen_scs_modelica(typdoc,%gd)

   for i=1:size(%gd.lang,1)

      tt=[];

      for j=1:size(list_of_modelica,1)

         //** generate tex head
         head_tex=get_head_tex(list_of_modelica(j,:),typdoc,i,%gd)

         //** change title of html head
         if %gd.lang(i)=='fr' then
            head_tex=strsubst(head_tex,'Fonction Scilab','Modelica')
         elseif %gd.lang(i)=='eng' then
            head_tex=strsubst(head_tex,'Scilab Function','Modelica')
         end

         name=get_extname(list_of_modelica(j,:),%gd)
...
//gen_scs_cdgen : main generator for scitexgendoc of tex files
//for the list_of_cdgen
function gen_scs_cdgen(typdoc,%gd)

   for i=1:size(%gd.lang,1)

      tt=[];

      for j=1:size(list_of_cdgen,1)

         //** generate tex head
         head_tex=get_head_tex(list_of_cdgen(j,:),typdoc,i,%gd)

         //** change title of html head
         if %gd.lang(i)=='fr' then
            head_tex=strsubst(head_tex,'Fonction Scilab','Generateur de Code Scicos')
         elseif %gd.lang(i)=='eng' then
            head_tex=strsubst(head_tex,'Scilab Function','Scicos Code Generation')
         end

         name=get_extname(list_of_cdgen(j,:),%gd)
...
function gen_scicos_doc(my_list,typdoc,%gd)
  ...
  gen_scs_modelica(typdoc,%gd);
  gen_scs_cdgen(typdoc,%gd);
  gen_scs_about_help(typdoc,%gd);
  generate_html_file(my_list,%gd);
  gen_scicos_whatis(%gd);
endfunction

//gen_scicos_one : generate scicos doc for one (or more) entity(ies)
function gen_scicos_one(my_list,typdoc,%gd)
  import_data_to_file('desc',%gd);
  outline_txt=generate_scs_outline();
  mputl(outline_txt,%gendoc.mpath.data(1)+'/whatis_scicos.xml');
  generate_aux_tex_file(my_list,typdoc,%gd);
  import_data_to_file('spdesc',%gd);
  generate_aux_tex_file(my_list,typdoc,%gd);
  generate_html_file(my_list,%gd);
  //gen_scicos_whatis(%gd);
endfunction
...
opath2=SCI+"/macros/"+...
       ["scicos"                      //1
        ...
        "scicos_blocks/ModElectrical" //3
        "scicos_blocks/Events"        //4
        "scicos_blocks/ModHydraulics" //5
        ...
        "scicos_blocks/Iterators"     //15
        "scicos_blocks/ModLinear"     //16
       ]+"/";
...
//util macros
listf_of_utilsci=[utilpath,"add_palette.sci","sci";
                  utilpath,"create_palette.sci","sci";
                  utilpath,"create_gif.sci","sci";
                  utilpath,"get_scicos_version.sci","sci"
                  utilpath,"getscicosparam.sci","sci"
                  utilpath,"setscicosparam.sci","sci"
                  palpath,"SetString.sci","sci";];

//auto macros
listf_of_autosci=[autopath,"scicos.sci","sci";
                  autopath,"scicos_simulate.sci","sci";
                  autopath,"scicos_codegeneration.sci","sci";
...
listf = basename(listfiles(opath2(5)+"*.sci"))+".sci";
for i = 1:size(listf,1)
  listf_of_thermo=[listf_of_thermo;
                   opath2(5),listf(i),"mblock"];
end
clear i;clear listf;
listf_of_modlin=[palpath,"ModLinear.cosf","pal"];
listf = basename(listfiles(opath2(16)+"*.sci"))+".sci";
for i = 1:size(listf,1)
  listf_of_modlin=[listf_of_modlin;
                   opath2(16),listf(i),"mblock"];
end

listf_of_modelicapal=[palpath,"Modelica.cosf","pal"];

listf_of_modelicablk=list([palpath,"Modelica.cosf","pal"],...
                          [opath2(7),"MBLOCK.sci","block"],...
                          [opath2(7),"MPBLOCK.sci","block"],...
                          listf_of_elec,...
                          listf_of_thermo,...
                          listf_of_modlin);
...
//Iterators palette
listf_of_iter=[palpath,"Iterators.cosf","pal"];
listf = basename(listfiles(opath2(15)+"*.sci"))+".sci";
for i = 1:size(listf,1)
    listf_of_iter=[listf_of_iter;
                   opath2(15),listf(i),"block"];
end
clear i;clear listf;
...
for i = 1:size(listf,1)
  if listf(i)<>'m_sin.sci' & listf(i)<>'DSUPER.sci' & ...
        listf(i)<>'bplatform2.sci' & listf(i)<>'anim_pen.sci' then
...
//Lookup_Tables palette
listf_of_lut=[palpath,"Lookup_Tables.cosf","pal"];
...
//**-- modelica --*/
list_of_modelica  = ["","ModelicaBlocks","sci";
                     "","iGUI","sci"];

//**-- CodeGeneration --*/
list_of_cdgen = ["","CodeGeneration","sci"];
//**------------*/
...
//**-- User manual --*/
list_of_user_manual=["","Context","sci";
                     "","iGUI","sci";
                     "","Setup_Scicos","sci";
                     "","CodeGeneration","sci";
                     listf_of_misc(find(listf_of_misc(:,2)=='DEBUG_SCICOS.sci'),:)];
