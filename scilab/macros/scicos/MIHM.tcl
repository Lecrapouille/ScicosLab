...
proc MIHM_Init { {WindowsID -1 } } {
    ...
    $mx add command  -label "Save XML for simulation" -underline 0 -accelerator {Alt+s} -command "button_SaveXMLs  $WindowsID_new"  
    ...
    $mx add command  -label "User Manual" -underline 0 -command "button_HelpiGUI  $WindowsID_new"
...
proc Compile { WindowsID } {
    ...
    if { $is_square =="false" } {
    	...
    	set dlg1 "The number of equations does not match the number of variables + relaxed parameters + discrete variables."
	set dlg2 "\nYou may need to relax or fix some variables or parameters."
	set dlg3 "\nIf you have discrete variables in your model, you need to specify their initialization equations in the <initiail equation> section of the model."
	set dlg4 "\n\rDo you want to continue anyway?"

	set answer [tk_messageBox -icon question -type yesno -title "$Model_name is not square" -message $dlg1$dlg2$dlg3$dlg4]
	if { $answer == "no" } {
	    Compute_finished noki $WindowsID 
	    return
	}
    }
...
proc Compile_finished { res WindowsID } {
	...
	Compute_finished nokc $WindowsID
    }
}
...
proc button_HelpWindow  { WindowsID }   { 
    set messegedlg "Modelica initialization GUI,\n\rThis version is used to initialize pure static Modelica models.\rREF: Initialization of Modelica Models in Scicos, M. Najafi, R. Nikoukhah.\r2008 Modelica Conference.  \n\rKeep us posted: scicos@inria.fr\r"
    tk_messageBox -icon question -type ok -title Help  -message "$messegedlg"
}

proc button_HelpiGUI  { WindowsID }   {
  ScilabEval "help(\"iGUI\");"
}
...
proc replace_ders_in_tree { WindowsID tree parent NewDerWeight NewStateWeight } {
   ...
   set kids [$tree nodes $parent]
	...
	for { set i 0 } { $i < $NRow } { incr i } {
	    ...
	    set fixed_origi  [lindex $rowi 11] ; # "fixed_orig" column
...
global scilabpath
	...
	set ModelName [string trim [string map {#text ""} $nm] "\{ \}"]
        ...
        if { [catch {$tree insert end $xparent $Main_node -image [image create photo -file "$scilabpath/macros/scicos/scicos2.gif"] -text $ModelName }] } {	    
	...
	set ans1 [string match "OutPutPort*" $StructName]
	set ans2 [string match "InPutPort*" $StructName]
	if { $ans1=="1" || $ans2=="1" } { 
	    if { [catch {$tree insert end $xparent $SVname -text "" -selectable "false"}] } {  return "" }
	} else {
	    if { [catch {$tree insert end $xparent $SVname  -image [image create photo -file "$scilabpath/macros/scicos/scicos.gif"] -text $StructName }] } {  return "" }
	}   
    ...
    #================================================================   
   	#tk_messageBox -icon error -type ok -title "in"  -message "$TrName,$TrFixedOrig,$TrFixed,$TrWeight"
   	if { $TrFixedOrig== "" } {	
   	  set TrFixedOrig $TrFixed 
   	  if { $TrFixedOrig == "" } {set  TrFixedOrig  "-"}
   	}  	  
 	  if { $TrFixed== "" } {	
   	  set TrFixed  $TrFixedOrig 
   	}  
    #=============================================
...
proc get_model_info { WindowsID } {
	...
	$w.nunknown configure -text "Unknowns=$NVAR" -fg red 
	set square false
    } else {
	$w.nequation configure -text "Equation=$NEQ" -fg blue
	$w.nunknown configure -text "Unknowns=$NVAR" -fg blue
	set square true
    }
