  ...
  for j=1:n //loop on objects
    ...
    if typeof(o)=='Block' then
      ...
      if o.gui=='CBLOCK4' then
        ...
        if or(omod.sim(1)==Changeb) then
          dep_t=omod.dep_ut($);dep_u(1:size(omod.in,'*'))=%t
          //printf("Update dep_ut for %s\n",omod.sim(1))
          //scs_m_new.objs(j).model.dep_ut=[dep_u' dep_t]
...
function scs_m_new=do_version272(scs_m)
  ...
  for i=1:lstsize(scs_m.objs)
	...
	rpar=do_version(scs_m_new.objs(i).model.rpar,'scicos2.7.1')
...
function scs_m_new=do_version271(scs_m)
  ...
  for i=1:n //loop on objects
    ...
    if typeof(o)=='Block' then
      omod=o.model;
      if omod.sim=='super'|omod.sim=='csuper' then
	rpar=do_version(omod.rpar,'scicos2.7')
	setfield($+1,0,omod)
	omod.rpar=rpar
...
for k=2:nx
  ...
  if o(1)=='Block' then
    ...
    if model(1)=='super'|model(1)=='csuper' then
      model(8)=do_version(model(8),'scicos2.4')
      o(3)=model
      scs_m(k)=o
...
function scs_m_new=do_version27(scs_m)
  ...
  for i=2:n //loop on objects
	...
	if type(mdl(8))==15 then
	  mdl(8)=do_version(mdl(8),'scicos2.5.1')
	end
      end
