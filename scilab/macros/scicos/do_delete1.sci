...
while K<>[] do
    ...
    if typ=='Link' then
	 ...
	 if or(scs_m.objs(to(1)).graphics.pin==k) & to(3)==1 then
      ...
      if or(fromblock.gui==['SPLIT_f' 'CLKSPLIT_f' 'IMPSPLIT_f' 'BUSSPLIT']) then
	...
	if size(setdiff(connected,K),'*')==2 then
	  ...
	  if find(connected(2)==DEL)<>[] then // delete split
	    ...
	    if gr==%t then
	      ...
	      gh_object.children(1).data = [o1.xx , o1.yy];
            end
      ...
      if toblock.gui=='SPLIT_f'|toblock.gui=='CLKSPLIT_f'|toblock.gui=='BUSSPLIT' then //
