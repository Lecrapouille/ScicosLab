  ...
  if rhs<=0 then,
    ...
    select nc
	  ...
	  [ok,fl,forder]=getvalue('Input filter characteristics',...
				  ['cut-off frequency (0.<frequ<.5):';'filter length'],...
				  list('vec',1,'vec',1),[' ';' '])
	  ftype='lp'
	  fh=0;
      case 2 then //high pass
	  [ok,fl,forder]=getvalue('Input filter characteristics',...
				  ['cut-off frequency (0.<frequ<.5):';'filter length (odd value)'],...
				  list('vec',1,'vec',1),[' ';' '])
	  fh=0;
	  flag=1;
	  ftype='hp'
	  ...
	  [ok,fl,fh,forder]=getvalue('Input filter characteristics',...
				     ['low cut-off frequency (0.<flow<.5):';
				      'high cut-off frequency (0.<flow<fhi<.5):';'filter length'],...
				     list('vec',1,'vec',1,'vec',1),[' ';' ';' '])
	  ftype='bp'	
      case 4 then //stop band
	  [ok,fl,fh,forder]=getvalue('Input filter characteristics',...
				     ['low cut-off frequency (0.<flow<.5):';
				      'high cut-off frequency (0.<flow<fhi<.5):';'filter length (odd value)'],...
				     list('vec',1,'vec',1,'vec',1),[' ';' ';' '])
	  flag=1;
	  ftype='sb'	
      else
