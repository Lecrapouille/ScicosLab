...
(* $Id: transl.ml 4846 2013-04-30 17:53:18Z weis $ *)
...
let string_of_unop = function
  ...
  | Ueye -> "mtlb_eye"
  | op ->
    try Hashtbl.find unop_inv_table op with
    | Not_found -> assert false
;;
...
let binop_of_string s =
  try Hashtbl.find binop_table s
  with
  | Not_found -> assert false
;;

let string_of_binop op =
  try Hashtbl.find binop_inv_table op
  with
  | Not_found -> assert false
;;
...
let ternop_of_string s =
  try Hashtbl.find ternop_table s
  with
  | Not_found -> assert false
;;

let string_of_ternop op =
  try Hashtbl.find ternop_inv_table op
  with
  | Not_found -> assert false
;;
...
let quaternop_of_string s =
  try Hashtbl.find quaternop_table s
  with
  | Not_found -> assert false
;;
...
and ( * ) e1 e2 = D_binary (Bmul, e1, e2)
and ( / ) e1 e2 = D_binary (Bdiv, e1, e2)
and ( ^ ) e1 e2 = D_binary (Bexp, e1, e2)
;;
...
let rec transl i =
  ...
  let rec loop exp =
    ...
    | E_string n -> D_string n
    | E_matrix expss ->
      D_matrix (List.map (List.map loop) expss)
    ...
    | E_apply (e_f, e_args) ->
      ...
      | [ e_arg1; e_arg2; ] ->
        D_ternary (Textract,
           e_f, transl (Aleft e_f) e_arg1, transl (Aright e_f) e_arg2)
      | _ ->
        let nargs = List.length e_args in
        transl_error
          (Printf.sprintf
